...
export default class {
  constructor(api) {
    ...
    this.CurrentConsumption = function CurrentConsumption() {
      ...
      this.setProps({
        format: api.Formats.UINT16,
        unit: 'W',
        maxValue: 100000,
        minValue: 0,
        minStep: 1,
        perms: [api.Perms.READ, api.Perms.NOTIFY],
      });
    ...
    this.ResetTotal = function ResetTotal() {
      ...
      this.setProps({
        format: api.Formats.UINT32,
        unit: api.Units.seconds,
        perms: [api.Perms.READ, api.Perms.NOTIFY, api.Perms.WRITE],
      });
      this.value = this.getDefaultValue();
    };
