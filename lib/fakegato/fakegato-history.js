...
module.exports = function (homebridge) {
  ...
  class FakeGatoHistory extends Service {
    ...
    registerEvents () {
      this.log('[%s] FG history: registering events.', thisAccessory.displayName)
      ...
      this.service.getCharacteristic(S2R2Characteristic).onGet(() => {
        return this.getCurrentS2R2()
      })
    ...
    _addEntry (entry) {
      if (this.loaded) {
        ...
        this.log(
          '[%s] FG history: first entry [%s] last entry [%s] used memory [%s] 116 [%s].',
          this.accessoryName,
          ...
          if (!err) {
            if (data) {
              try {
                ...
                this.log('FG history: error fetching data - invalid JSON [%s].', e)
                cb(e, false)
              }
              cb(null, true)
            }
      ...
      if (this.currentEntry <= this.lastEntry && this.transfer) {
        ...
        for (let i = 0; i < 11; i++) {
          if (
            ...
            this.log(
              '[%s] FG history: entry [%s] address [%s].',
              this.accessoryName,
              this.currentEntry,
              this.memAddress
            )
        ...
        this.log('[%s] FG history: data [%s].', this.accessoryName, this.dataStream)
    ...
    setCurrentS2W1 (value) {
      this.log('[%s] FG history: data req [%s].', this.accessoryName, base64ToHex(value))
      ...
      this.log('[%s] FG history: address req [%s].', this.accessoryName, hexAddress)
      this.sendHistory(address)
    }

    setCurrentS2W2 (value) {
      this.log('[%s] FG history: clock adj [%s].', this.accessoryName, base64ToHex(value))
    }
  }
