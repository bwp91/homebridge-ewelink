...
module.exports = function (pHomebridge, log) {
  ...
  const hexToBase64 = val => Buffer.from(('' + val).replace(/[^0-9A-F]/ig, ''), 'hex').toString('base64')
  ...
  class FakeGatoHistory extends Service {
      ...
      switch (accessoryType) {
        ...
        case 'door':
          ...
          homebridge.globalFakeGatoTimer.subscribe(this, function (params) {
            const actualEntry = {}
            if ((params.backLog || []).length) {
              actualEntry.status = params.backLog[0].status
              actualEntry.time = params.immediate ? params.backLog[0].time : Math.round(new Date().valueOf() / 1000)
              this.service.log('[%s] fakegato cb [%s]', this.service.accessory.displayName, actualEntry)
              this.service._addEntry(actualEntry)
            }
          })
          break
        case 'motion':
        ...
        case 'custom':
          ...
          this.accessoryType116 = ' 0' +
            ...
            ' ' +
            this.signatures.sort((a, b) => (a.signature > b.signature) ? 1 : -1).map(a => a.signature).join(' ') +
            ' '
          homebridge.globalFakeGatoTimer.subscribe(this, this.calculateAverage)
          break
        case 'aqua':
    ...
    _addEntry (entry) {
      if (this.loaded) {
        ...
        this.service.setCharacteristic(S2R1Characteristic, hexToBase64(val))
        this.log(
