...
module.exports = class eWeLinkPlatform {
  ...
  initialiseDevice (device) {
    ...
    try {
      ...
      if (helpers.devicesCurtain.includes(device.extra.uiid)) {
        ...
        *********************/
        if (this.config.resetRFBridge) {
          this.devicesInHB
            .filter(a => helpers.hasProperty(a, 'context') && a.context.eweDeviceId === device.deviceId)
            .foreach(a => this.removeAccessory(a))
          this.log.warn('[%s] has been reset.', device.name)
        }
        let rfChlCounter = 0
        const rfMap = []
        ...
        rfMap.forEach(subDevice => {
          ...
          if (['1', '2', '3', '4'].includes(subDevice.type)) {
            ...
            if (this.cusS.has(device.deviceid + 'SW' + swNumber)) {
              sensorTimeLength = helpers.hasProperty(this.cusS.get(device.deviceid + 'SW' + swNumber), 'sensorTimeLength')
                ? this.cusS.get(device.deviceid + 'SW' + swNumber).sensorTimeLength
                : undefined
            }
          } else {
            return
  ...
  async sendDeviceUpdate (accessory, params) {
    ...
    const res = helpers.devicesNonLAN.includes(accessory.context.eweUIID)
      ? "device doesn't support LAN mode"
