...
module.exports = class eWeLinkPlatform {
    ...
    try {
      ...
      if (helpers.devicesCurtain.includes(device.extra.uiid)) {
        ...
        ***********************/
        ...
        accessory.control = new deviceSwitchMulti(this, accessory)
  ...
  async sendDeviceUpdate (accessory, params, doAwait = true) {
    if (doAwait) {
      // *** generates a random number in the range [10, 1000] *** \\
      await helpers.sleep(Math.floor(Math.random() * (991) + 10))
    }
    const payload = {
