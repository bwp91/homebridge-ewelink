  ...
  constructor (config, log, devices) {
    ...
    devices.forEach(device => {
      this.deviceMap.set(device.deviceid, {
        ...
        online: !!utils.hasProperty(this.ipOverrides, device.deviceid),
        ip: utils.hasProperty(this.ipOverrides, device.deviceid) ? this.ipOverrides[device.deviceid] : null
      })
    })
  }
  ...
  async getHosts () {
    ...
    res.forEach(device => {
      ...
      if ((d = this.deviceMap.get(deviceId))) {
        if (!utils.hasProperty(this.ipOverrides, deviceId)) {
      ...
      if (packet.answers) {
        packet.answers
          ...
          .forEach(value => {
            ...
            let params
            if (packet.address !== deviceInfo.ip && !utils.hasProperty(this.ipOverrides, rdata.id)) {
