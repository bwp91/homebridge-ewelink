...
module.exports = class eWeLinkLAN {
  constructor (config, log, devices) {
    ...
    devices.forEach(device => {
      this.deviceMap.set(device.deviceid, {
        ...
        online: utils.hasProperty(this.config.ipOverride, device.deviceid),
        ip: utils.hasProperty(this.config.ipOverride, device.deviceid) ? this.config.ipOverride[device.deviceid] : null
      })
    })
  }
  ...
  async getHosts () {
    ...
    res.forEach(device => {
      ...
      if ((d = this.deviceMap.get(deviceId))) {
        if (!utils.hasProperty(this.config.ipOverride, deviceId)) {
      ...
      if (packet.answers) {
        packet.answers
          ...
          .forEach(value => {
            ...
            const data =
              rdata.data1 +
              (utils.hasProperty(rdata, 'data2') ? rdata.data2 : '') +
              (utils.hasProperty(rdata, 'data3') ? rdata.data3 : '') +
              (utils.hasProperty(rdata, 'data4') ? rdata.data4 : '')
            ...
            let params
            if (packet.address !== deviceInfo.ip && !utils.hasProperty(this.config.ipOverride, rdata.id)) {
            ...
            for (const param in params) {
              if (utils.hasProperty(params, param)) {
  ...
  async sendUpdate (json) {
    try {
      ...
      let suffix
      const params = {}
      if (utils.hasProperty(json.params, 'switches')) {
        params.switches = json.params.switches
        suffix = 'switches'
      } else if (utils.hasProperty(json.params, 'switch')) {
      ...
      if ((apiKey = this.deviceMap.get(json.deviceid).apiKey)) {
        ...
        if (!utils.hasProperty(res.data, 'error') || res.data.error !== 0) {
