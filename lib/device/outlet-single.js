...
module.exports = class deviceOutletSingle {
  constructor (platform, accessory) {
    ...
    /*
    UIID 1/6/14/24/27/112: single switch, no power readings
    ...
    */
    ...
    let deviceConf
    if (platform.outletDevices[accessory.context.eweDeviceId]) {
      deviceConf = platform.outletDevices[accessory.context.eweDeviceId]
    } else if (platform.singleDevices[accessory.context.eweDeviceId]) {
      deviceConf = platform.singleDevices[accessory.context.eweDeviceId]
    }
    ...
    switch (this.accessory.context.eweUIID) {
      ...
      default:
        ...
        if (platform.consts.devices.outletSCM.includes(this.accessory.context.eweUIID)) {
          this.isSCM = true
        }
    ...
    if (this.powerReadings) {
      // Get the in-use threshold from the device config
  ...
  async externalUpdate (params) {
    try {
      if (!this.isSCM && params.switch && params.switch !== this.cacheState) {
        ...
        if (this.powerReadings && this.cacheState === 'off') {
          this.service.updateCharacteristic(this.hapChar.OutletInUse, false)
          this.service.updateCharacteristic(this.eveChar.CurrentConsumption, 0)
          this.accessory.eveService.addEntry({ power: 0 })
          this.accessory.context.energyReadings.push(0)
