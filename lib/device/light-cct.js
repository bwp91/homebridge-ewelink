...
module.exports = class deviceLightCCT {
  constructor (platform, accessory) {
    ...
    this.deviceModel = deviceConf && deviceConf.deviceModel ? deviceConf.deviceModel : 'bulbB02FA60'
    this.alShift =
    ...
    this.offlineAsOff = deviceConf && deviceConf.offlineAsOff
  ...
  async externalUpdate (params) {
    try {
      ...
      if (params.white) {
        if (this.funcs.hasProperty(params.white, 'br') && this.cacheBright !== params.white.br) {
  ...
  markStatus (isOnline) {
    ...
    // Show as off if offline if configured by user
