    ...
    try {
      ...
      if (!this.disableDeviceLogging) {
        if (this.alController && this.alController.isAdaptiveLightingActive()) {
          ...
          this.log('[%s] current cct [%sK].', this.dName, mToK)
        }
      }
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
  ...
  async externalUpdate (params) {
    try {
      ...
      if (
        ...
        if (this.isB1) {
          this.service.updateCharacteristic(this.C.Brightness, this.cacheOnOff === 'on' ? 100 : 0)
        }
        ...
        case 22:
          ...
          if (mode === 2) {
            ...
            if (params.updateSource) {
              this.log('[%s] current mode [white] is unsupported.', this.dName)
            }
          }
          break
        case 59:
        ...
        case 104:
          ...
          if (params.ltype === 'white' && params.white) {
            ...
            if (this.helpers.hasProperty(params.white, 'ct') && params.white.ct !== this.cacheCT) {
              ...
              if (params.updateSource) {
                if (!this.disableDeviceLogging) {
                  this.log('[%s] current cct [%sK].', this.dName, ctToK)
                }
                if (
                  this.alController &&
