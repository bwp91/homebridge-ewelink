  ...
  async internalCTempUpdate (value, callback) {
    try {
      ...
      if (this.alController && this.alController.isAdaptiveLightingActive()) {
        this.log('[%s] current cct [%s mired] via adaptive lighting.', this.accessory.displayName, value)
      } else {
        this.log('[%s] current cct [%s mired].', this.accessory.displayName, value)
      }
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
    }
  ...
  async externalUpdate (params) {
    try {
      ...
      if (
        ...
        if (this.isB1) this.lightService.updateCharacteristic(this.Characteristic.Brightness, this.cacheOnOff === 'on' ? 100 : 0)
