...
module.exports = class deviceLightColour {
  ...
  async internalBrightnessUpdate (value, callback) {
    try {
      callback()
      if (this.cacheBrightness === value) return
      let params
  ...
  async internalColourUpdate (value, callback) {
    try {
      ...
      this.cacheR = this.accessory.context.eweUIID === 22 ? newRGB[0].toString() : newRGB[0]
      this.cacheG = this.accessory.context.eweUIID === 22 ? newRGB[1].toString() : newRGB[1]
      this.cacheB = this.accessory.context.eweUIID === 22 ? newRGB[2].toString() : newRGB[2]
