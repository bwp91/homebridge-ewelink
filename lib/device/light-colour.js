...
module.exports = class deviceLightColour {
  ...
  async internalCTempUpdate (value, callback) {
    try {
      ...
      this.accessory.context.updateCTemp = updateKeyCTemp
      // *** B02-B-A60, B05-B-A60, GTLC104 *** \\
      ...
      const kToCT = Math.round(((kelvin - 2200) / 3800) * 255)
      const params = {
        ltype: 'white',
        white: {
          ct: kToCT,
      ...
      await this.helpers.sleep(350)
      if (updateKeyCTemp !== this.accessory.context.updateKeyCTemp) return
      await this.platform.sendDeviceUpdate(this.accessory, params)
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
    }
            ...
            if (this.helpers.hasProperty(params, 'ltype')) {
              ...
              if (mode === 'color' && this.helpers.hasProperty(params, 'color')) {
                ...
                if (this.helpers.hasProperty(params.white, 'ct')) {
                  // HomeKit has a ct range of 140-500 corresponding to 2000-7143K
                  // These devices have a range of 2700K-6500K corresponding to ct: 0-255
                  const ctToK = Math.round(params.white.ct / 255 * 3800 + 2200)
