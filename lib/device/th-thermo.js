    ...
    try {
      ...
      if (this.helpers.hasProperty(params, 'currentTemperature') && params.currentTemperature !== 'unavailable') {
        ...
        if (this.cacheTemp !== currentTemp) {
          ...
          this.service.updateCharacteristic(this.Characteristic.TargetTemperature, this.cacheTemp)
          if (params.updateSource) this.log('[%s] current temperature [%s].', this.accessory.displayName, this.cacheTemp)
        }
      }
      if (
        ...
        if (this.cacheHumi !== currentHumi) {
          ...
          this.service.updateCharacteristic(this.Characteristic.CurrentRelativeHumidity, this.cacheHumi)
          if (params.updateSource) this.log('[%s] current humidity [%s%].', this.accessory.displayName, this.cacheHumi)
        }
      }
