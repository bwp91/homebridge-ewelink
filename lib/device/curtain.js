...
module.exports = class deviceCurtain {
  constructor (platform, accessory) {
    ...
    const showAsMotor =
      deviceConf && deviceConf.showAsMotor
        ? deviceConf.showAsMotor
        : platform.consts.defaultValues.showAsMotor
  ...
  async externalUpdate (params) {
    try {
      ...
      if (this.isGarage) {
        if (this.funcs.hasProperty(params, 'op') && this.funcs.hasProperty(params, 'per')) {
          if (params.op === 3 && params.per === 0) {
            ...
            if (this.cachePos !== 1) {
              ...
              if (params.updateSource && this.enableLogging) {
                this.log('[%s] %s [%s].', this.name, 'current state', 'closed')
              }
            }
            ...
            if (this.cachePos !== 0) {
              ...
              if (params.updateSource && this.enableLogging) {
                this.log('[%s] %s [%s].', this.name, 'current state', 'open')
              }
            }
          }
        }
