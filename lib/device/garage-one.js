...
module.exports = class deviceGarageOne {
  ...
  async internalUpdate (value, callback) {
    try {
      ...
      const prevState = this.definedSensor
        ? this.definedSensor.getService(this.Service.ContactSensor).getCharacteristic(this.Characteristic.ContactSensorState).value === 0
          ? 1
          : 0
      ...
      if (this.setup === 'oneSwitch' && [2, 3].includes(prevState)) {
        this.service.updateCharacteristic(this.Characteristic.CurrentDoorState, ((prevState * 2) % 3) + 2)
      ...
      this.service.updateCharacteristic(this.Characteristic.TargetDoorState, newPos)
      ...
      if (!this.definedSensor) {
        this.service.updateCharacteristic(this.Characteristic.CurrentDoorState, newPos)
  ...
  async externalUpdate (params) {
    try {
      ...
      this.inUse = true
      this.service.updateCharacteristic(this.Characteristic.TargetDoorState, newPos - 2)
      ...
      this.inUse = false
      await this.helpers.sleep(Math.max((this.operationTime - 20) * 100, 0))
      this.service.updateCharacteristic(this.Characteristic.CurrentDoorState, newPos - 2)
      this.accessory.context.cacheCurrentDoorState = newPos - 2
      if (!this.disableDeviceLogging) {
