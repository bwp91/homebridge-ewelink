...
export default class {
  constructor(platform, accessory) {
    ...
    [0, 1, 2, 3, 4, 5].forEach((channel) => {
      const label = channel + 1;
      ...
      if (!this[`service${label}`]) {
        ...
        this[`service${label}`].updateCharacteristic(this.hapChar.ServiceLabelIndex, channel);
      }

      this.timeouts[channel] = false;
    });
