...
module.exports = class deviceLightColour {
  constructor (platform, accessory) {
    ...
    if ((this.platform.config.bulbB02FST64 || '').split(',').includes(accessory.context.eweDeviceId)) {
      this.minK = 1800
      this.maxK = 5000
    } else if ((this.platform.config.bulbB02BA60 || '').split(',').includes(accessory.context.eweDeviceId)) {
      this.minK = 2700
      this.maxK = 6500
    } else {
      this.minK = 2200
    ...
    try {
      ...
      if (params.white) {
        ...
        if (this.helpers.hasProperty(params.white, 'ct') && this.cacheCT !== params.white.ct) {
          ...
          this.service.updateCharacteristic(this.Characteristic.ColorTemperature, this.cacheMired)
