        ...
        case 36:
          ...
          if (this.helpers.hasProperty(params, 'bright')) {
            ...
            if (nb !== this.cacheBrightness) {
              this.service.updateCharacteristic(this.C.Brightness, nb)
              this.cacheBrightness = nb
              if (params.updateSource && !this.disableDeviceLogging) {
                this.log('[%s] current brightness [%s%].', this.dName, nb)
              }
            }
          }
          break
        case 44:
          ...
          if (this.helpers.hasProperty(params, 'brightness')) {
            if (params.brightness !== this.cacheBrightness) {
              this.service.updateCharacteristic(this.C.Brightness, params.brightness)
              this.cacheBrightness = params.brightness
              if (params.updateSource && !this.disableDeviceLogging) {
                this.log('[%s] current brightness [%s%].', this.dName, params.brightness)
              }
            }
          }
          break
