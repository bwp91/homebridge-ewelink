...
module.exports = class deviceRFBridge {
  constructor (platform, accessory) {
    ...
    if (['curtain', 'button'].includes(accessory.context.subType)) {
      ...
      accessory.eveService = new this.platform.eveService(eveType, accessory)
    }
    ...
    try {
      ...
      if (this.helpers.hasProperty(params, 'cmd') && params.cmd === 'transmit' && this.helpers.hasProperty(params, 'rfChl')) {
        ...
        if (oAccessory) {
          ...
          if (params.updateSource) this.log('[%s] current state [button triggered]', oAccessory.displayName)
          ...
          .forEach(async chan => {
            ...
            if (oAccessory) {
              ...
              if (diff < this.sensorTimeDifference) {
                ...
                if (params.updateSource) this.log('[%s] current state [sensor triggered]', oAccessory.displayName)
