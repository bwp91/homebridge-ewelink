...
module.exports = class deviceRFBridge {
  constructor (platform, accessory, devicesInHB) {
    ...
    const deviceConf = platform.rfDevices[accessory.context.eweDeviceId]
        ...
        Object.keys(params)
          ...
          .forEach(async chan => {
            ...
            if (subAccessory) {
              ...
              let sensorType
              if (['button', 'curtain'].includes(subAccessory.context.subType)) {
                sensorType = subAccessory.context.subType
              } else {
                sensorType = deviceConf && deviceConf.type ? deviceConf.type : 'motion'
              }
              ...
              let serv
              let char
              if (diff < sensorTimeDifference || ['button', 'curtain'].sensorType) {
                switch (sensorType) {
                  case 'button':
                  case 'curtain': {
                ...
                // The doorbell is the one service that doesn't follow the pattern of activation
                if (serv === this.hapServ.Doorbell) {
