...
module.exports = class deviceRFBridge {
  constructor (platform, accessory) {
    ...
    this.sensorTimeLengthPri = parseInt(accessory.context.sensorTimeLength)
    this.sensorTimeLengthSec = parseInt(this.platform.config.sensorTimeLength)
    this.sensorTimeLength = isNaN(this.sensorTimeLengthPri)
      ? isNaN(this.sensorTimeLengthSec)
        ? helpers.defaults.sensorTimeLength
        : this.sensorTimeLengthSec < 0
          ? helpers.defaults.sensorTimeLength
          : this.sensorTimeLengthSec
      : this.sensorTimeLengthPri < 0
        ? isNaN(this.sensorTimeLengthSec)
          ? helpers.defaults.sensorTimeLength
          : this.sensorTimeLengthSec < 0
            ? helpers.defaults.sensorTimeLength
            : this.sensorTimeLengthSec
        : this.sensorTimeLengthPri
    if (helpers.hasProperty(accessory.context, 'sensorTimeDifference')) {
      this.sensorTimeLength = isNaN(this.sensorTimeLength)
        ? helpers.defaults.sensorTimeLength
        : this.sensorTimeLength < 0
          ? helpers.defaults.sensorTimeLength
    ...
    switch (accessory.context.subType) {
      ...
      case 'contact': {
        ...
        if (!this.platform.config.disableEveLogging) {
          ...
          corrInterval.setCorrectingInterval(() => {
            const dataToAdd = {
              time: Date.now(),
              status: contactService.getCharacteristic(this.Characteristic.ContactSensorState).value
            }
      ...
      case 'occupancy':
        ...
        break
      default: {
        const motionService = accessory.getService(this.Service.MotionSensor) || accessory.addService(this.Service.MotionSensor)
        if (!this.platform.config.disableEveLogging) {
          accessory.log = this.platform.log
          ...
          corrInterval.setCorrectingInterval(() => {
            const dataToAdd = {
              time: Date.now(),
              status: motionService.getCharacteristic(this.Characteristic.MotionDetected).value ? 1 : 0
            }
            accessory.eveLogger.addEntry(dataToAdd)
          }, 600000)
        }
          ...
          .forEach(async chan => {
            ...
            if (oAccessory) {
              ...
              if (diff < this.sensorTimeDifference) {
                ...
                await helpers.sleep(this.sensorTimeLength * 1000)
