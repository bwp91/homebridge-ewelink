...
module.exports = class deviceOutlet {
  constructor (platform, accessory) {
    ...
    this.inherits(this.eveCurrentConsumption, this.C)
    this.inherits(this.eveTotalConsumption, this.C)
    this.inherits(this.eveVoltage, this.C)
    this.inherits(this.eveElectricCurrent, this.C)
    this.inherits(this.eveResetTotal, this.C)
    ...
    if (!this.helpers.hasProperty(this.accessory.context, 'energyReadings')) {
      this.accessory.context.energyReadings = []
    }
    if (!this.helpers.hasProperty(this.accessory.context, 'energyReadingTotal')) {
    ...
    try {
      ...
      if (params.updateSource && logger && !this.disableDeviceLogging) {
        this.log(
          ...
          this.helpers.hasProperty(params, 'voltage') ? ' voltage [' + params.voltage + 'V]' : '',
          this.helpers.hasProperty(params, 'current') ? ' current [' + params.current + 'A]' : ''
        )
      }
    } catch (err) {
