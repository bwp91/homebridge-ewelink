...
module.exports = class deviceOutlet {
  constructor (platform, accessory) {
    ...
    this.pollingInterval = deviceConf && deviceConf.pollingInterval
      ...
      : platform.consts.defaultValues.pollingInterval
    ...
    // Set up the polling interval if the user has enabled it
    /*
    if (this.pollingInterval > 0) {
      this.intervalPoll = setInterval(() => {
        platform.wsClient.requestUpdate(this.accessory)
      }, this.pollingInterval * 1000)
    }
    */

    if (this.consts.devices.outlet.includes(this.accessory.context.eweUIID)) {
      setTimeout(() => {
        this.internalUIUpdate()
        this.intervalPoll = setInterval(() => this.internalUIUpdate, 1800000)
      }, 5000)
    }
  ...
  async internalUIUpdate () {
    try {
      await this.platform.sendDeviceUpdate(this.accessory, { uiActive: 1800 }, true)
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
    }
  }
    ...
    try {
      if (params.switch && params.switch !== this.cacheOnOff) {
        ...
        if (!this.accessory.context.powerReadings) {
          this.service
            .updateCharacteristic(this.hapChar.OutletInUse, params.switch === 'on')
        }
      }
      let logger = false
      ...
      if (this.funcs.hasProperty(params, 'current')) {
        this.service
          .updateCharacteristic(this.eveElectricCurrent, parseFloat(params.current))
        logger = true
      }
