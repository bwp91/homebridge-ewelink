...
module.exports = class deviceZBDev {
    ...
    switch (this.accessory.context.eweUIID) {
      ...
      case 1256:
        ...
        this.accessory.getService(this.S.Switch).getCharacteristic(this.C.On)
          .on('set', this.internalSwitchOnOffUpdate.bind(this))
        break
      case 1257:
        if (!this.accessory.getService(this.S.Lightbulb)) {
          this.accessory.addService(this.S.Lightbulb)
        }
        this.accessory.getService(this.S.Lightbulb).getCharacteristic(this.C.On)
  ...
  async internalSwitchOnOffUpdate (value, callback) {
    try {
      callback()
