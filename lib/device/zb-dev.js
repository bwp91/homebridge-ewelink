      ...
      switch (this.accessory.context.eweUIID) {
        ...
        case 1256:
          if (this.helpers.hasProperty(params, 'switch') && ['off', 'on'].includes(params.switch)) {
            ...
            this.accessory.eveLogger.addEntry({ status: params.switch === 'on' ? 1 : 0 })
          }
          break
        case 1770: {
          const eveLog = {}
          if (this.helpers.hasProperty(params, 'temperature')) {
            ...
            if (this.debugDevice && params.updateSource) {
              this.log('[%s] reported temperture [%s].', this.accessory.displayName, currentTemp)
            }
          ...
          if (this.helpers.hasProperty(params, 'humidity')) {
            ...
            if (this.debugDevice && params.updateSource) {
              this.log('[%s] reported humidity [%s].', this.accessory.displayName, currentHumi)
            }
          }
        ...
        case 2026:
          if (this.helpers.hasProperty(params, 'motion') && this.helpers.hasProperty(params, 'trigTime')) {
            ...
            this.accessory.eveLogger.addEntry({ status: motionDetected ? 1 : 0 })
            if (motionDetected) {
        ...
        case 3026:
          if (this.helpers.hasProperty(params, 'lock') && [0, 1].includes(params.lock)) {
            ...
            this.accessory.eveLogger.addEntry({ status: params.lock })
          }
          if (params.lock) {
