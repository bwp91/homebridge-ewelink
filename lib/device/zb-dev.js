      ...
      switch (this.accessory.context.eweUIID) {
        ...
        case 1256:
          if (this.helpers.hasProperty(params, 'switch') && ['off', 'on'].includes(params.switch)) {
            ...
            if (params.updateSource) {
              this.log('[%s] current status [%s].', this.accessory.displayName, params.switch)
            }
        ...
        case 2026:
          if (this.helpers.hasProperty(params, 'motion') && this.helpers.hasProperty(params, 'trigTime')) {
            ...
            if (params.updateSource) {
              this.log('[%s] current status [motion%s detected].', this.accessory.displayName, motionDetected ? '' : ' not')
            }
            if (motionDetected) {
        ...
        case 3026:
          if (this.helpers.hasProperty(params, 'lock') && [0, 1].includes(params.lock)) {
            ...
            if (params.updateSource) {
              this.log('[%s] current status [contact%s detected].', this.accessory.displayName, params.lock === 0 ? '' : ' not')
            }
