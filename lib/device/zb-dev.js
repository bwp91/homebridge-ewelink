...
module.exports = class deviceZBDev {
  constructor (platform, accessory) {
    ...
    switch (accessory.context.eweUIID) {
      case 1000: {
        if (!accessory.getService(this.S.BatteryService)) accessory.addService(this.S.BatteryService)
        const zbspsService =
          accessory.getService(this.S.StatelessProgrammableSwitch) ||
          accessory.addService(this.S.StatelessProgrammableSwitch)
      ...
      case 1256:
        if (accessory.getService(this.S.BatteryService)) {
          accessory.removeService(accessory.getService(this.S.BatteryService))
        }
        if (!accessory.getService(this.S.Switch)) accessory.addService(this.S.Switch)
        ...
        accessory.getService(this.S.Switch).getCharacteristic(this.C.On)
          .on('set', this.internalUpdate.bind(this))
        break
      case 1770: {
        if (!accessory.getService(this.S.BatteryService)) accessory.addService(this.S.BatteryService)
        const tempService = accessory.getService(this.S.TemperatureSensor) || accessory.addService(this.S.TemperatureSensor)
        tempService.getCharacteristic(this.C.CurrentTemperature)
          .setProps({ minValue: -100 })
        if (!accessory.getService(this.S.HumiditySensor)) accessory.addService(this.S.HumiditySensor)
        accessory.log = platform.config.debugFakegato ? this.log : () => {}
        accessory.historyService = new this.platform.eveService('weather', accessory, {
          storage: 'fs',
          path: platform.eveLogPath
        })
        break
      ...
      case 2026:
        if (!accessory.getService(this.S.BatteryService)) accessory.addService(this.S.BatteryService)
        if (!accessory.getService(this.S.MotionSensor)) accessory.addService(this.S.MotionSensor)
        accessory.log = platform.config.debugFakegato ? this.log : () => {}
        accessory.historyService = new this.platform.eveService('motion', accessory, {
          storage: 'fs',
          path: platform.eveLogPath
        })
        break
      case 3026:
        if (!accessory.getService(this.S.BatteryService)) accessory.addService(this.S.BatteryService)
        if (!accessory.getService(this.S.ContactSensor)) accessory.addService(this.S.ContactSensor)
        accessory.log = platform.config.debugFakegato ? this.log : () => {}
        accessory.historyService = new this.platform.eveService('door', accessory, {
          storage: 'fs',
          path: platform.eveLogPath
        })
        break
      ...
      if (params.battery) {
        ...
        if (params.battery !== this.cacheBattery) {
          ...
          if (params.updateSource && !this.disableDeviceLogging) {
            this.log('[%s] current battery [%s%].', this.accessory.displayName, params.battery)
          }
        }
      }
      switch (this.accessory.context.eweUIID) {
        ...
        case 1770: {
          ...
          if (params.temperature && params.temperature !== this.cacheTemp) {
            ...
            if (params.updateSource && !this.disableDeviceLogging) {
              this.log('[%s] current temperature [%s].', this.accessory.displayName, currentTemp)
            }
        ...
        case 2026:
          if (this.helpers.hasProperty(params, 'motion') && this.helpers.hasProperty(params, 'trigTime')) {
            ...
            if (params.updateSource && !this.disableDeviceLogging) {
              this.log('[%s] current state [motion%s detected].', this.accessory.displayName, motionDetected ? '' : ' not')
            }
            if (motionDetected) {
        ...
        case 3026:
          if (this.helpers.hasProperty(params, 'lock') && [0, 1].includes(params.lock)) {
            ...
            if (params.updateSource && !this.disableDeviceLogging) {
              this.log('[%s] current state [contact%s detected].', this.accessory.displayName, params.lock === 0 ? '' : ' not')
            }
            this.accessory.historyService.addEntry({
              time: Math.round(new Date().valueOf() / 1000),
              status: params.lock
