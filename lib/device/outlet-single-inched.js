  ...
  constructor (platform, accessory) {
    ...
    */
    ...
    this.service.getCharacteristic(this.cusChar.InvertSwitch).onSet(value => {
      if (value) {
        const curValue = this.service.getCharacteristic(this.hapChar.On).value
        this.service.updateCharacteristic(this.hapChar.On, !curValue)
        this.cacheState = curValue ? 'off' : 'on'
        this.log('[%s] %s [%s].', this.name, this.lang.curState, this.cacheState)
        setTimeout(() => {
          this.service.updateCharacteristic(this.cusChar.InvertSwitch, false)
        }, 3000)
      }
    })
