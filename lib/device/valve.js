...
module.exports = class deviceValve {
  constructor (platform, accessory) {
    ...
    ;['A', 'B'].forEach(v => {
      let valveService
      const valveConfig = this.platform.cusG.get(this.accessory.context.hbDeviceId)
      if (!(valveService = this.accessory.getService('Valve ' + v))) {
        ...
        valveService = this.accessory.getService('Valve ' + v)
      }
      valveService
  ...
  async internalUpdate (valve, value, callback) {
    ...
    try {
      ...
      serviceValve
        .updateCharacteristic(this.Characteristic.Active, value)
