...
module.exports = class deviceDiffuser {
  constructor (platform, accessory) {
    ...
    this.fanService = this.accessory.getService('Diffuser') || this.accessory.addService(this.S.Fan, 'Diffuser', 'diffuser')
    this.lightService = this.accessory.getService('Light') || this.accessory.addService(this.S.Lightbulb, 'Light', 'light')
    this.fanService.getCharacteristic(this.C.On)
      .on('set', this.internalDiffuserOnOffUpdate.bind(this))
    this.fanService.getCharacteristic(this.C.RotationSpeed)
    ...
    this.lightService.getCharacteristic(this.C.Brightness)
      .on('set', this.internalLightBrightnessUpdate.bind(this))
    this.lightService.getCharacteristic(this.C.Hue)
      .on('set', this.internalLightColourUpdate.bind(this))
    this.lightService.getCharacteristic(this.C.Saturation)
  ...
  async internalDiffuserSpeedUpdate (value, callback) {
    try {
      ...
      await this.platform.sendDeviceUpdate(this.accessory, params)
      if (!this.disableDeviceLogging) {
        this.log('[%s] current speed [%s%].', this.dName, newSpeed)
      }
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
    }
  ...
  async internalLightOnOffUpdate (value, callback) {
    try {
      ...
      await this.platform.sendDeviceUpdate(this.accessory, params)
      if (!this.disableDeviceLogging) {
        this.log('[%s] current light [%s%].', this.dName, value ? 'on' : 'off')
      }
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
    }
  ...
  async internalLightColourUpdate (value, callback) {
    try {
      ...
      const newRGB = this.colourUtils.hs2rgb(
        value,
        this.accessory.getService('Light').getCharacteristic(this.C.Saturation).value
      )
      ...
      await this.platform.sendDeviceUpdate(this.accessory, params)
      if (!this.disableDeviceLogging) {
        this.log('[%s] current colour [rgb %s %s %s].', this.dName, this.cacheR, this.cacheG, this.cacheB)
      }
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
    }
  ...
  async externalUpdate (params) {
    try {
      ...
      if (params.switch && params.switch !== this.cacheOnOff) {
        ...
        this.fanService.updateCharacteristic(this.C.On, this.cacheOnOff === 'on')
        if (!this.helpers.hasProperty(params, 'state')) {
          this.fanService.updateCharacteristic(this.C.RotationSpeed, this.cacheSpeed)
        }
      }
      ...
      if (this.helpers.hasProperty(params, 'lightswitch') && this.cacheLightOnOff !== params.lightswitch) {
        ...
        this.lightService.updateCharacteristic(this.C.On, params.lightswitch === 1)
        if (params.updateSource && !this.disableDeviceLogging) {
          this.log('[%s] current light [%s%].', this.dNameparams.lightswitch === 1 ? 'on' : 'off')
        }
      }
      if (this.helpers.hasProperty(params, 'lightbright') && this.cacheBrightness !== params.lightbright) {
        ...
        this.lightService.updateCharacteristic(this.C.Brightness, params.lightbright)
        if (params.updateSource && !this.disableDeviceLogging) {
          this.log('[%s] current brightness [%s%].', this.dNameparams.lightbright)
        }
      }
      if (this.helpers.hasProperty(params, 'lightRcolor')) {
        if (this.cacheR !== params.lightRcolor || this.cacheG !== params.lightGcolor || this.cacheB !== params.lightBcolor) {
          ...
          this.lightService.updateCharacteristic(this.C.Hue, this.cacheHue)
