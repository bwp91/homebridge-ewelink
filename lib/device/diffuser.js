...
module.exports = class deviceDiffuser {
  ...
  async internalDiffuserSpeedUpdate (value) {
    try {
      ...
      // Log the update if appropriate
      if (!this.disableDeviceLogging) {
        this.log('[%s] %s [%s%].', this.name, this.messages.curSpeed, this.cacheSpeed)
      }
    } catch (err) {
      // Catch any errors and let the platform display them
      this.platform.deviceUpdateError(this.accessory, err, true)
  ...
  async internalLightOnOffUpdate (value) {
    try {
      ...
      if (!this.disableDeviceLogging) {
        ...
        this.log('[%s] %s [%s%].', this.name, this.messages.curLight, label)
      }
    } catch (err) {
      // Catch any errors and let the platform display them
      this.platform.deviceUpdateError(this.accessory, err, true)
  ...
  async externalUpdate (params) {
    try {
      ...
      if (
        ...
        // Log if appropriate
        if (params.updateSource && !this.disableDeviceLogging) {
          const label = params.lightswitch === 1 ? 'on' : 'off'
          this.log('[%s] %s [%s%].', this.name, this.messages.curLight, label)
        }
      }
