...
module.exports = class deviceDiffuser {
  constructor (platform, accessory) {
    ...
    this.fanService
      ...
      .onSet(async value => await this.internalDiffuserOnOffUpdate(value))

    // Add the set handler to the fan rotation characteristic
    this.fanService
      .getCharacteristic(this.hapChar.RotationSpeed)
      .setProps({ minStep: 50 })
      .onSet(async value => await this.internalDiffuserSpeedUpdate(value))
  ...
  async internalDiffuserSpeedUpdate (value) {
    try {
      ...
      const updateKeySpeed = this.funcs.generateRandomString(5)
  ...
  async internalLightStateUpdate (value) {
    try {
      ...
      // Set up a five second timeout for the plugin to ignore incoming updates
      const updateKeyLight = this.funcs.generateRandomString(5)
