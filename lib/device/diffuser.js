...
module.exports = class deviceDiffuser {
  constructor (platform, accessory) {
    ...
    this.lightService.getCharacteristic(this.hapChar.On).onSet(async value => {
      await this.internalLightOnOffUpdate(value)
    })
  ...
  async internalDiffuserSpeedUpdate (value) {
    try {
      ...
      const newValue = value <= 75 ? 50 : 100
      ...
      this.fanService.updateCharacteristic(this.hapChar.RotationSpeed, newValue)
  ...
  async internalLightOnOffUpdate (value) {
    try {
      // Don't continue if the new value is the same as before
      const newValue = value ? 1 : 0
      if (this.cacheLightOnOff === newValue) {
        return
      }
      ...
      this.cacheLightOnOff = newValue

      // Log the update if appropriate
      if (!this.disableDeviceLogging) {
        this.log('[%s] current light [%s%].', this.name, value === 1 ? 'on' : 'off')
      }
    } catch (err) {
      // Catch any errors and let the platform display them
      this.platform.deviceUpdateError(this.accessory, err, true)
