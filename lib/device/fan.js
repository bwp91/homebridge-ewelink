...
module.exports = class deviceFan {
  constructor (platform, accessory) {
    ...
    fanService
      ...
      .on('set', (value, callback) => {
        callback()
        if (!value) fanService.setCharacteristic(this.Characteristic.RotationSpeed, 0)
      })
    fanService
  ...
  async internalUpdate (accessory, type, value, callback) {
    ...
    try {
      ...
      let newLight
      let lightService
      if (this.visibleLight) lightService = accessory.getService(this.Service.Lightbulb)
