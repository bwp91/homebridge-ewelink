...
module.exports = class deviceFan {
  constructor (platform, accessory) {
    ...
    this.service = accessory.getService(this.S.Fan) || accessory.addService(this.S.Fan)
    this.service.getCharacteristic(this.C.On)
      .on('set', (value, callback) => {
        callback()
        if (!value) this.service.setCharacteristic(this.C.RotationSpeed, 0)
      })
    ...
    if ((this.platform.config.hideLightFromFan || '').split(',').includes(accessory.context.eweDeviceId)) {
      if (accessory.getService(this.S.Lightbulb)) {
        accessory.removeService(accessory.getService(this.S.Lightbulb))
      }
    } else {
      this.lightService = accessory.getService(this.S.Lightbulb) || accessory.addService(this.S.Lightbulb)
  ...
  async externalUpdate (params) {
    try {
      ...
      if (this.lightService && params.switches[0].switch !== this.cacheLight) {
        ...
        if (params.updateSource && !this.disableDeviceLogging) {
          this.log('[%s] current light state [%s].', this.accessory.displayName, this.cacheLight)
        }
      }
    } catch (err) {
