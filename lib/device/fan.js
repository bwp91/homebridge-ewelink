...
module.exports = class deviceFan {
  ...
  async internalSpeedUpdate (value) {
    try {
      ...
      if (newSpeed !== this.cacheSpeed && newSpeed > 0) {
        ...
        if (!this.disableDeviceLogging) {
          const label = this.speed2label[this.cacheSpeed]
          this.log('[%s] %s [%s].', this.name, this.messages.curSpeed, label)
        }
      }
    } catch (err) {
  ...
  async internalLightUpdate (value) {
    try {
      ...
      if (!this.disableDeviceLogging) {
        this.log('[%s] %s [%s].', this.name, this.messages.curLight, this.cacheLight)
      }
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
      setTimeout(() => {
