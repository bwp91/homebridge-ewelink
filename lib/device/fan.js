...
module.exports = class deviceFan {
    ...
    fanService
      ...
      .on('set', async (value, callback) => {
        ...
        if (value === 0) {
          ...
          fanService.setCharacteristic(Characteristic.RotationSpeed, 0)
        }
      })
    fanService
  ...
  async internalUpdate (accessory, type, value, callback) {
    ...
    try {
      ...
      this.platform.log.error('updating %s characteristic', type)
      this.platform.log.error(
        'INTERNAL FAN UPDATE: light [%s] power [%s] speed [%s%]',
        newLight ? 'on' : 'off',
        newPower ? 'on' : 'off',
        newSpeed
      )
  ...
  async externalUpdate (accessory, params) {
    try {
      if (params.switches && Array.isArray(params.switches)) {
        ...
        this.platform.log.error('**********************************************')
        this.platform.log.error(
          'EXTERNAL FAN UPDATE: light [%s] power [%s] speed [%s%]',
          light ? 'on' : 'off',
          status ? 'on' : 'off',
          speed
        )
