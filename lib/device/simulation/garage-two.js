...
module.exports = class deviceGarageTwo {
  constructor (platform, accessory) {
    ...
    this.operationTime = platform.simulations[eweId].operationTime
      ...
      switch (garage) {
        case 'Garage 1': {
          ...
          if (!this.disableDeviceLogging) {
            this.log('[%s] current state [garage 1 %s].', this.name, newPos === 0 ? 'open' : 'closed')
          }
          break
        }
        case 'Garage 2': {
          ...
          if (!this.disableDeviceLogging) {
            this.log('[%s] current state [garage 2 %s].', this.name, newPos === 0 ? 'open' : 'closed')
          }
          break
        }
      }
        ...
        switch (v) {
          case '1':
            ...
            if (params.updateSource && !this.disableDeviceLogging) {
              this.log('[%s] current state [garage 1 %s].', this.name, newPos === 2 ? 'open' : 'closed')
            }
            break
          case '2':
            this.accessory.context.cacheTwoCurrentDoorState = newPos - 2
            if (params.updateSource && !this.disableDeviceLogging) {
              this.log('[%s] current state [garage 2 %s].', this.name, newPos === 2 ? 'open' : 'closed')
            }
            break
        }
      })
