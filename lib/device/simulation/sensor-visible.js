    ...
    try {
      ...
      for (const [deviceId, group] of Object.entries(this.platform.deviceConf)) {
        if (
          ...
          ['garage', 'lock'].includes(group.type)
        ) {
          ...
          if (this.devicesInHB.has(uuid)) {
            ...
            const subService = subAccessory.getService(
              group.type === 'garage' ? this.hapServ.GarageDoorOpener : this.hapServ.LockMechanism
            )
            ...
            switch (newState) {
              case 0:
                if (isGarage) {
                  ...
                  subService.setCharacteristic(this.hapChar.LockCurrentState, 1)
                  if (params.updateSource && enableLogging) {
              ...
              case 1: {
                if (isGarage) {
                  ...
                  subService.setCharacteristic(this.hapChar.LockCurrentState, 0)
                  if (params.updateSource && enableLogging) {
