    ...
    try {
      ...
      switch (tap) {
        case 'Tap A':
          ...
          params.switches[1].switch = this.accessory.getService('Tap B').getCharacteristic(this.hapChar.Active).value === 1
            ? 'on'
            : 'off'
          break
        case 'Tap B':
          params.switches[0].switch = this.accessory.getService('Tap A').getCharacteristic(this.hapChar.Active).value === 1
            ? 'on'
            : 'off'
      ...
      if (!this.disableDeviceLogging) {
        this.log('[%s] current state [%s %].', this.name, tap, value === 1 ? 'running' : 'stopped')
      }
  ...
  async externalUpdate (params) {
    try {
      ...
      ;['A', 'B'].forEach((v, k) => {
        ...
        tapService.updateCharacteristic(this.hapChar.Active, params.switches[k].switch === 'on' ? 1 : 0)
          .updateCharacteristic(this.hapChar.InUse, params.switches[k].switch === 'on' ? 1 : 0)
