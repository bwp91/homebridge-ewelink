...
module.exports = class deviceHeater {
  constructor (platform, accessory) {
    ...
    if (deviceConf2 && deviceConf2.overrideLogging) {
      switch (deviceConf2.overrideLogging) {
        case 'standard':
    ...
    // Set the humidifier as the primary service
    this.service.setPrimaryService()

    // Add a get handler to the current temperature characteristic
    ...
    // Add a get handler to the current state characteristic
    ...
    // Add a get handler to the current relative humidity characteristic
    ...
    this.service
      ...
      .onGet(() => {
        ...
        return this.service.getCharacteristic(this.hapChar.RelativeHumidityHumidifierThreshold)
          .value
      })
    ...
    this.cacheHumid =
      this.cacheState === 'on' &&
      this.service.getCharacteristic(this.hapChar.TargetHumidifierDehumidifierState).value === 2
        ? 'on'
        : 'off'
  ...
  async internalStateUpdate (value) {
    try {
      ...
      this.service.updateCharacteristic(
        ...
        value === 1 ? (this.cacheHumid === 'on' ? 2 : 1) : 0
      )
    } catch (err) {
