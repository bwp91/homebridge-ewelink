...
module.exports = class deviceGarageEachen {
  constructor (platform, accessory) {
    ...
    if (!(this.service = this.accessory.getService(this.S.GarageDoorOpener))) {
      this.service = this.accessory.addService(this.S.GarageDoorOpener)
    ...
    try {
      ...
      if (value === 0) {
        ...
        if (!this.disableDeviceLogging) {
          this.log('[%s] current state [open].', this.dName)
        }
      }
    } catch (err) {
      this.inUse = false
  ...
  async externalUpdate (params) {
    try {
      ...
      if (params.updateSource && !this.disableDeviceLogging) {
        this.log('[%s] current state [%s].', this.dName, params.switch === 'on' ? 'open' : 'closed')
      }
    } catch (err) {
