...
module.exports = class deviceGarageOne {
  constructor (platform, accessory, devicesInHB) {
    ...
    if (deviceConf.sensorId) {
      ...
      const uuid = this.hapUUIDGen(deviceConf.sensorId + 'SWX')
      if (!devicesInHB.has(uuid)) {
        this.error = this.lang.simErrNoSensor
      }

      // Check the sensor is a sensor
      if (!platform.consts.devices.garageSensors.includes(devicesInHB.get(uuid).context.eweUIID)) {
        this.error = this.lang.simErrNotSensor
      }
      this.definedSensor = devicesInHB.get(uuid)
