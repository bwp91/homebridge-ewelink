...
module.exports = class deviceBlind {
  constructor (platform, accessory) {
    ...
    this.operationTimeDown = deviceConf.operationTimeDown || this.operationTime
  ...
  async internalUpdate (value, callback) {
    try {
      ...
      const percentStepUpPerDS = this.operationTime / 100
      ...
      if (prevState !== 2) {
        ...
        if (prevState === 0) {
          // Was going down
          prevPosition -= Math.floor(percentStepDownPerDS * posPercentChange)
        } else {
          // Was going up
          prevPosition += Math.floor(percentStepUpPerDS * posPercentChange)
        }
      ...
      const decisecondsToMove = setToMoveUp
        ? Math.round(Math.abs(diffPosition) * percentStepUpPerDS)
        : Math.round(Math.abs(diffPosition) * percentStepDownPerDS)
