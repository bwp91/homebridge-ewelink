...
module.exports = class deviceCooler {
  constructor (platform, accessory) {
    ...
    // Set the cooler as the primary service
    ...
    // Add the set handler to the cooler active characteristic
    ...
    // Setting this now makes the plugin send the payload to setup as a cooler
    setTimeout(() => {
  ...
  async internalStateUpdate (value) {
    try {
      ...
      if (value === 0) {
        ...
        this.cacheCool = this.cacheTemp > this.accessory.context.cacheTarget ? 'on' : 'off'
        params.switch = this.cacheCool
      }
  ...
  async externalUpdate (params) {
    try {
      ...
      if (params.switch && params.switch !== this.cacheCool) {
        this.cacheCool = params.switch
        this.service.updateCharacteristic(
          this.hapChar.CurrentHeaterCoolerState,
          this.cacheCool === 'on' ? 3 : this.cacheState === 'on' ? 1 : 0
        )
