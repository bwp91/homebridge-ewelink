...
module.exports = class deviceTapTwo {
  constructor (platform, accessory) {
    ...
    ;['A', 'B'].forEach(v => {
      let tapService
      if (!(tapService = this.accessory.getService(this.S.Valve))) {
        tapService = this.accessory.addService(this.S.Valve)
  ...
  async internalUpdate (tap, value, callback) {
    try {
      ...
      if (!this.disableDeviceLogging) {
        this.log('[%s] current state [%s %].', this.dName, tap, value === 1 ? 'running' : 'stopped')
      }
    ...
    try {
      ...
      ;['A', 'B'].forEach((v, k) => {
        ...
        if (params.updateSource && !this.disableDeviceLogging) {
          this.log('[%s] current state [Tap %s running].', this.dName, v, logText)
        }
      })
    } catch (err) {
