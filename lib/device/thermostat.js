  ...
  async externalUpdate (accessory, params) {
    try {
      if (
        ...
        (utils.hasProperty(params, 'switch') || utils.hasProperty(params, 'mainSwitch'))
      ) {
        const newState = utils.hasProperty(params, 'switch') ? params.switch === 'on' : params.mainSwitch === 'on'
        const switchService = accessory.getService(Service.Switch)
      ...
      if (utils.hasProperty(params, 'currentTemperature') && accessory.getService(Service.TemperatureSensor)) {
      ...
      if (utils.hasProperty(params, 'currentHumidity') && accessory.getService(Service.HumiditySensor)) {
