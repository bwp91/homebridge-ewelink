...
module.exports = class deviceUSB {
  constructor (platform, accessory) {
    ...
    if ((platform.config.outletAsSwitch || '').split(',').includes(accessory.context.eweDeviceId)) {
      if (accessory.getService(this.S.Outlet)) {
        accessory.removeService(accessory.getService(this.S.Outlet))
      }
      this.service = accessory.getService(this.S.Switch) || accessory.addService(this.S.Switch)
    } else {
      if (accessory.getService(this.S.Switch)) {
        accessory.removeService(accessory.getService(this.S.Switch))
      }
      this.service = accessory.getService(this.S.Outlet) || accessory.addService(this.S.Outlet)
    }
