    ...
    try {
      if (this.helpers.hasProperty(params, 'battery')) {
        ...
        if (this.cacheBattery !== scaledBattery) {
          this.batteryService
            .updateCharacteristic(this.Characteristic.BatteryLevel, this.cacheBattery)
            .updateCharacteristic(this.Characteristic.StatusLowBattery, this.cacheBattery < this.lowBattThreshold)
          if (params.updateSource) this.log('[%s] current battery [%s%].', this.accessory.displayName, this.cacheBattery)
        }
      }
          ...
          if ((oAccessory = this.platform.devicesInHB.get(group.deviceId + 'SWX'))) {
            ...
            switch (newState) {
              case 0:
                ...
                if (params.updateSource) this.log('[%s] current state [closed].', oAccessory.displayName)
                break
              case 1:
                ...
                if (params.updateSource) this.log('[%s] current state [open].', oAccessory.displayName)
                break
            }
          }
        }
