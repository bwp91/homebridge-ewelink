  ...
  async internalColourUpdate (value) {
    try {
      ...
      if (!this.disableDeviceLogging) {
        ...
        this.log('[%s] %s [rgb %s].', this.name, this.messages.curColour, label)
      }
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
      setTimeout(() => {
    ...
    try {
      ...
      if (!this.disableDeviceLogging) {
        if (this.alController.isAdaptiveLightingActive()) {
          ...
          this.log('[%s] %s [%sK].', this.name, this.messages.curColour, mToK)
        }
      }
    } catch (err) {
      this.platform.deviceUpdateError(this.accessory, err, true)
            ...
            if (
              ...
              if (params.updateSource) {
                ...
                if (this.alController.isAdaptiveLightingActive() && rgbDiff > 50) {
                  ...
                  this.log('[%s] %s.', this.name, this.messages.disabledAL)
                }
              }
            }
          }
        ...
        case 104:
          ...
          if (params.ltype === 'white' && params.white) {
            ...
            if (
              ...
              if (params.updateSource) {
                if (!this.disableDeviceLogging) {
                  this.log('[%s] %s [%sK].', this.name, this.messages.curColour, ctToK)
                }
