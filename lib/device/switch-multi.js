...
module.exports = class deviceSwitch {
  ...
  async internalUpdate (value, callback) {
    // *** generates a random number in the range [10, 100] *** \\
    await helpers.sleep(Math.floor(Math.random() * 91 + 10))
    if (this.updateInProgress) {
      await helpers.sleep(300)
      return await this.internalUpdate(value, callback)
    }
    this.updateInProgress = true
    setTimeout(() => (this.updateInProgress = false), 250)
    callback()
    try {
      let oAccessory
      let masterState = 'off'
