        ...
        case '0':
          ...
          for (let i = 0; i <= 4; i++) {
            if ((oAccessory = this.platform.devicesInHB.get(this.accessory.context.eweDeviceId + 'SW' + i))) {
              ...
              if (i > 0) this.log('[%s] current state [%s].', oAccessory.displayName, value ? 'on' : 'off')
            }
          }
          break
        case '1':
    ...
    try {
      ...
      for (let i = 1; i <= this.accessory.context.channelCount; i++) {
        ...
        if (this.platform.devicesInHB.has(idToCheck + i)) {
          ...
          if (params.updateSource) this.log('[%s] current state [%s].', oAccessory.displayName, params.switches[i - 1].switch)
        }
      }
      if (!this.platform.hiddenMasters.includes(this.accessory.context.eweDeviceId)) {
        this.service.updateCharacteristic(this.Characteristic.On, primaryState)
