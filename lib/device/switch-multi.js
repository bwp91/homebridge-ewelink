...
module.exports = class deviceSwitchMulti {
  constructor (platform, accessory, devicesInHB) {
    ...
    */
    ...
    this.inchChannels = deviceConf && deviceConf.inchChannels ? deviceConf.inchChannels : ''
    ...
    if (this.accessory.context.isInched) {
      ...
      this.service.getCharacteristic(this.cusChar.InvertSwitch).onSet(value => {
        if (value) {
          ...
          this.log('[%s] %s [%s].', this.name, this.lang.curState, curValue ? 'off' : 'on')
          setTimeout(() => {
