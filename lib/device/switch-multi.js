    ...
    try {
      ...
      switch (this.accessory.context.switchNumber) {
        case '0':
          ...
          for (let i = 0; i <= 4; i++) {
            if ((oAccessory = this.platform.devicesInHB.get(this.accessory.context.eweDeviceId + 'SW' + i))) {
              if (this.showAsOutlet) {
                oAccessory.getService(this.hapServ.Outlet).updateCharacteristic(this.hapChar.On, value)
                oAccessory.getService(this.hapServ.Outlet).updateCharacteristic(this.hapChar.OutletInUse, value)
              } else {
        ...
        case '4':
          ...
          for (let i = 1; i <= 4; i++) {
            if ((oAccessory = this.platform.devicesInHB.get(this.accessory.context.eweDeviceId + 'SW' + i))) {
              const serviceToUse = this.showAsOutlet ? this.hapServ.Outlet : this.hapServ.Switch
              if (oAccessory.getService(serviceToUse).getCharacteristic(this.hapChar.On).value) {
                masterState = 'on'
              }
          ...
          if (!this.platform.hideMasters.includes(this.accessory.context.eweDeviceId)) {
            ...
            if (this.showAsOutlet) {
              oAccessory.getService(this.hapServ.Outlet).updateCharacteristic(this.hapChar.On, masterState === 'on')
              oAccessory.getService(this.hapServ.Outlet).updateCharacteristic(this.hapChar.OutletInUse, masterState === 'on')
            } else {
    ...
    try {
      ...
      for (let i = 1; i <= this.accessory.context.channelCount; i++) {
        ...
        if (this.platform.devicesInHB.has(idToCheck + i)) {
          ...
          if (params.updateSource && params.switches[i - 1].switch === oAccessory.context.cacheOnOff) {
            continue
          }
          if (this.showAsOutlet) {
            ...
            oAccessory.getService(this.hapServ.Outlet)
              .updateCharacteristic(this.hapChar.OutletInUse, params.switches[i - 1].switch === 'on')
          } else {
