...
module.exports = class deviceGarageEachen {
  ...
  async internalUpdate (accessory, value, callback) {
    ...
    try {
      ...
      if (newPos === 0) {
        await utils.sleep(Math.max((garageConfig.operationTime - 30) * 100, 0))
        gdService.updateCharacteristic(Characteristic.CurrentDoorState, 1)
      }
    } catch (err) {
      accessory.context.inUse = false
  ...
  async externalUpdate (accessory, params) {
    try {
      ...
      if (params.switch === 'on') {
        await utils.sleep(Math.max(garageConfig.operationTime * 100, 3000))
      }
