...
module.exports = class deviceBlind {
  constructor (platform, accessory) {
    ...
    if (!(this.service = this.accessory.getService(this.S.WindowCovering))) {
      this.service = this.accessory.addService(this.S.WindowCovering)
  ...
  async internalUpdate (value, callback) {
    try {
      ...
      if (!this.disableDeviceLogging) {
        this.log('[%s] current position [%s%]', this.dName, value)
      }
    } catch (err) {
