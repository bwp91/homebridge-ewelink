...
module.exports = class deviceZBSensorMotion {
  constructor (platform, accessory) {
    ...
    // Add the get handler to the motion detected characteristic
    this.service.getCharacteristic(this.hapChar.MotionDetected).onGet(() => {
      if (this.isOnline) {
        return this.service.getCharacteristic(this.hapChar.MotionDetected).value
      } else {
  ...
  async externalUpdate (params) {
    try {
      ...
      if (
        ...
        if (params.updateSource && !this.disableDeviceLogging) {
          this.log(
            '[%s] current state [motion%s detected].',
            this.name,
            motionDetected ? '' : ' not'
          )
        }
