    ...
    try {
      if (
        ...
        if (params.updateSource && !this.disableDeviceLogging) {
          this.log('[%s] %s [%s%].', this.name, this.messages.curBatt, this.cacheBatt)
        }
      }
      if (this.funcs.hasProperty(params, 'key') && [0, 1, 2].includes(params.key)) {
        ...
        if (params.updateSource && !this.disableDeviceLogging) {
          const textLabel = params.key === 0
            ? this.messages.buttonSingle
            : (params.key === 1 ? this.messages.buttonDouble : this.messages.buttonLong)
          this.log('[%s] %s [%s].', this.name, this.messages.curState, textLabel)
        }
      }
    } catch (err) {
