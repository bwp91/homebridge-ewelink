...
export default class {
  constructor(platform, accessory) {
    ...
    // Add the occupancy sensor service if it doesn't already exist
    this.service = this.accessory.getService(this.hapServ.OccupancySensor);
    if (!this.service) {
      this.service = this.accessory.addService(this.hapServ.OccupancySensor);
  ...
  async externalUpdate(params) {
    try {
      ...
      if (
        hasProperty(params, 'human')
        && hasProperty(params, 'trigTime')
        && params.occupancy !== this.cacheState
      ) {
        this.cacheState = params.occupancy;
        ...
        this.service.updateCharacteristic(this.hapChar.OccupancyDetected, occupancyDetected);
        ...
        this.accessory.eveService.addEntry({ status: occupancyDetected ? 1 : 0 });
        if (params.updateSource && this.enableLogging) {
          this.log(
            '[%s] %s [%s].',
            this.name,
            this.lang.curState,
            occupancyDetected ? this.lang.occupancyYes : this.lang.occupancyNo,
          );
        }
      }
    } catch (err) {
