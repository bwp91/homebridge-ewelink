...
module.exports = class eWeLinkWS {
  ...
  login () {
    ...
    this.wsp.onOpen.addListener(async () => {
      ...
      try {
        ...
        if (helpers.hasProperty(res, 'config') && res.config.hb && res.config.hbInterval) {
    ...
    this.wsp.onUnpackedMessage.addListener(device => {
      ...
      if (!helpers.hasProperty(device, 'params')) device.params = {}
      if (helpers.hasProperty(device, 'deviceid') && helpers.hasProperty(device, 'error')) {
      ...
      if (helpers.hasProperty(device, 'action')) {
        switch (device.action) {
          case 'update':
          case 'sysmsg':
            if (device.action === 'sysmsg' && helpers.hasProperty(device.params, 'online')) {
              onlineStatus = device.params.online
            }
            for (const param in device.params) {
              if (helpers.hasProperty(device.params, param)) {
      ...
      } else if (helpers.hasProperty(device, 'error') && device.error === 0) {
  ...
  async sendUpdate (json) {
    ...
    if (this.wsp && this.wsIsOpen) {
      try {
        ...
        device.error = helpers.hasProperty(device, 'error') ? device.error : 504
        switch (device.error) {
          case 0:
            return
          default:
      ...
      await helpers.sleep(30000)
      return await this.sendUpdate(json)
    }
  }
  ...
  async requestUpdate (accessory) {
    ...
    if (this.wsp && this.wsIsOpen) {
      ...
      await helpers.sleep(30000)
      return await this.requestUpdate(accessory)
    }
  }
  ...
  async closeConnection () {
    if (this.wsp && this.wsIsOpen) {
      await this.wsp.close(1005, 'Normal Closure')
