...
module.exports = class eWeLinkLAN {
  constructor (config, log, devices) {
    ...
    devices.forEach(device => {
      this.deviceMap.set(device.deviceid, {
        ...
        online: helpers.hasProperty(this.config.ipOverride, device.deviceid),
        ip: helpers.hasProperty(this.config.ipOverride, device.deviceid) ? this.config.ipOverride[device.deviceid] : null
      })
    })
  }
  ...
  async getHosts () {
    ...
    res.forEach(device => {
      ...
      if ((d = this.deviceMap.get(deviceId))) {
        if (!helpers.hasProperty(this.config.ipOverride, deviceId)) {
      ...
      if (packet.answers) {
        packet.answers
          ...
          .forEach(value => {
            ...
            const data =
              rdata.data1 +
              (helpers.hasProperty(rdata, 'data2') ? rdata.data2 : '') +
              (helpers.hasProperty(rdata, 'data3') ? rdata.data3 : '') +
              (helpers.hasProperty(rdata, 'data4') ? rdata.data4 : '')
            ...
            let params
            if (packet.address !== deviceInfo.ip && !helpers.hasProperty(this.config.ipOverride, rdata.id)) {
            ...
            for (const param in params) {
              if (helpers.hasProperty(params, param)) {
                if (!helpers.paramsToKeep.includes(param.replace(/[0-9]/g, ''))) {
                  delete params[param]
                }
              }
            }
  ...
  async sendUpdate (json) {
    try {
      ...
      let suffix
      const params = {}
      if (helpers.hasProperty(json.params, 'switches')) {
        params.switches = json.params.switches
        suffix = 'switches'
      } else if (helpers.hasProperty(json.params, 'switch')) {
      ...
      if ((apiKey = this.deviceMap.get(json.deviceid).apiKey)) {
        ...
        if (!helpers.hasProperty(res.data, 'error') || res.data.error !== 0) {
