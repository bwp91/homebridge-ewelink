...
class eWeLink {
  constructor (log, config, api) {
    ...
    this.ipOverrideTmp = {}
    this.config.nameOverride.forEach(x => this.nameOverrideTmp[x.key] = x.value)
    this.config.ipOverride.forEach(x => this.ipOverrideTmp[x.key] = x.value)
    ...
    try {
      ...
      if (cns.devicesCurtain.includes(device.extra.uiid)) {
        ...
        **********************************/
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX', false, cns.defaultGarageTwoCache)
        ...
        *********************/
        if ((this.config.hideChanFromHB.split(',') || []).includes(device.deviceid)) {
