  ...
  login () {
    ...
    this.wsp.onOpen.addListener(async () => {
      ...
      try {
        ...
        if (utils.hasProperty(res, 'config') && res.config.hb && res.config.hbInterval && !this.hbInterval) {
    ...
    this.wsp.onUnpackedMessage.addListener(device => {
      ...
      if (!utils.hasProperty(device, 'params')) device.params = {}
      if (utils.hasProperty(device, 'deviceid') && utils.hasProperty(device, 'error')) {
      ...
      if (utils.hasProperty(device, 'action')) {
        switch (device.action) {
          case 'update':
          case 'sysmsg':
            if (device.action === 'sysmsg' && utils.hasProperty(device.params, 'online')) {
              onlineStatus = device.params.online
            }
            for (const param in device.params) {
              if (utils.hasProperty(device.params, param)) {
      ...
      } else if (utils.hasProperty(device, 'error') && device.error === 0) {
  ...
  async sendUpdate (json) {
    ...
    if (this.wsp && this.wsIsOpen) {
      try {
        ...
        device.error = utils.hasProperty(device, 'error') ? device.error : 504
        switch (device.error) {
          case 0:
            return
          default:
