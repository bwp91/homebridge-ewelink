...
class eWeLinkPlatform {
  ...
  async eWeLinkSetup () {
    try {
      ...
      this.lanClient = new (require('./connection/lan'))(this.config, this.log, this.helpers, this.ipOverride, deviceList)
      ...
      this.wsRefresh = setInterval(async () => {
        try {
          ...
          this.log.warn(this.debug || this.debugReqRes ? err : err.message)
        }
      }, 3600000)
  ...
  async sendDeviceUpdate (accessory, params) {
    ...
    if (this.debugReqRes) {
      this.log.warn('[%s] sending update (yellow text for clarity).\n%s', accessory.displayName, JSON.stringify(params, null, 2))
    } else if (this.debug) {
      this.log('[%s] sending update.', accessory.displayName)
    }
    ...
    if (res !== 'ok') {
      ...
      if (accessory.context.reachableWAN) {
        if (this.debug || this.debugReqRes) this.log('[%s] Reverting to web socket as %s.', accessory.displayName, res)
  ...
  async receiveDeviceUpdate (device) {
    ...
    if ((accessory = this.devicesInHB.get(deviceId + 'SWX') || this.devicesInHB.get(deviceId + 'SW0'))) {
      if (this.debugReqRes) {
        this.log('[%s] update received.\n%s', accessory.displayName, JSON.stringify(device.params, null, 2))
      } else if (this.debug) {
        this.log('[%s] update received via %s.', accessory.displayName, device.params.updateSource)
      }
      ...
      try {
        ...
        if ((this.config.hideDevFromHB || '').split(',').includes(deviceId)) return

        if (this.debugReqRes) {
          this.log('[%s] update received for new device.\n%s', device.name, JSON.stringify(device.params, null, 2))
        } else if (this.debug) {
          this.log('[%s] update received for new device via %s.', device.name, device.params.updateSource)
        }
  ...
  async deviceUpdateError (accessory, err, requestRefresh) {
    ...
    if (accessory.context.reachableWAN) {
      try {
        ...
        this.log.warn('[%s] requesting previous state information to revert.', accessory.displayName)
      } catch (err) {}
    } else {
      this.log.warn('[%s] state will be synced when device comes back online.', accessory.displayName)
    }
  }
