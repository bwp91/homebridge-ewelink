...
class eWeLinkPlatform {
  constructor (log, config, api) {
    ...
    try {
      ...
      this.ignoredDevices = (this.config.ignoredDevices || '').replace(/[\s'"]+/g, '').toLowerCase().split(',')
      this.thTempOffset = {}
      this.nameOverride = {}
      this.ipOverride = {}
  ...
  async eWeLinkSetup () {
    try {
      ...
      if (Object.keys(this.config.groups || []).length > 0) {
        this.config.groups
          .filter(g => g.type && this.helpers.allowedGroups.includes(g.type))
          .filter(g => g.deviceId && this.devicesInEW.has(g.deviceId.toLowerCase()))
          .forEach(g => {
            this.cusG.set(g.deviceId.toLowerCase(), g)
            if (['garage', 'garage_eachen'].includes(g.type) && g.obstructId && this.devicesInEW.has(g.obstructId.toLowerCase())) {
              this.obstructSwitches[g.obstructId.toLowerCase()] = g.deviceId.toLowerCase()
            }
          })
      }
      ...
      if (Object.keys(this.config.bridgeSensors || []).length > 0) {
        this.config.bridgeSensors
          ...
          .forEach(s => this.cusS.set(s.fullDeviceId.toLowerCase(), s))
      }
    ...
    try {
      ...
      if (this.helpers.devicesCurtain.includes(device.extra.uiid)) {
        ...
        *****************************/
        ...
        accessory.control = new (require('./device/garage-od-switch'))(this, accessory)
        /****************************/
        ...
        ***************************/
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX')
        accessory.control = new (require('./device/lock-one'))(this, accessory)
        /**************************/
        ...
        ********************************/
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX')
        accessory.control = new (require('./device/tap-two'))(this, accessory)
        /*******************************/
        ...
        ***********************************/
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX')
        accessory.control = new (require('./device/valve-four'))(this, accessory)
        /**********************************/
      ...
      this.log.warn('[%s] could not be initialised as %s.', device.name, this.debug ? err : err.message)
    }
  }
  ...
  addAccessory (device, hbDeviceId, hidden = false, extraContext = {}, type = '') {
    ...
    try {
      ...
      this.log.warn('[%s] could not be added to Homebridge as %s.', newDeviceName, err)
      return false
    }
  }
  ...
  removeAccessory (accessory) {
    try {
      ...
      this.log.warn('[%s] could not removed from Homebridge as %s.', accessory.displayName, err)
    }
  }
  ...
  async deviceUpdateError (accessory, err, requestRefresh) {
    this.log.warn('[%s] device update failed as %s on line %s.', accessory.displayName, err.message, err.lineNumber)
