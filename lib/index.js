              ...
              for (const [k, v] of Object.entries(x)) {
                switch (k) {
                  ...
                  case 'hideLight':
                    ...
                    this.fanDevices[deviceId][k] = v === 'false' ? false : !!v
                    break
                  default:
                    logRemove(key + '.' + k)
                    break
              ...
              for (const [k, v] of Object.entries(x)) {
                switch (k) {
                  ...
                  case 'deviceId':
                    break
                  case 'bulbModel': {
                    const inSet = this.consts.allowed.bulbModels.includes(v)
                    if (typeof v !== 'string' || !inSet) {
                      logIgnore(k)
                    } else {
                      this.lightDevices[deviceId][k] = inSet
                        ? v
                    ...
                    if (typeof v !== 'string' || v === '') {
                      ...
                      channels.forEach(channel => {
                        this.hideChannels.push(
                          deviceId + 'SW' + channel.replace(/[^0-9]+/g, '')
                        )
                      })
                    }
                    break
              ...
              for (const [k, v] of Object.entries(x)) {
                switch (k) {
                  ...
                  case 'inUsePowerThreshold': {
                    ...
                    if (isNaN(intVal)) {
                      logDefault(key + '.' + k, this.consts.defaultValues[k])
                      this.outletDevices[deviceId][k] = this.consts.defaultValues[k]
                    } else if (intVal < this.consts.minValues[k]) {
                      logIncrease(key + '.' + k, this.consts.minValues[k])
                      this.outletDevices[deviceId][k] = this.consts.minValues[k]
                    } else {
                      this.outletDevices[deviceId][k] = intVal
                    }
                    break
                  }
                  case 'ipAddress': {
                  ...
                  case 'showAsSwitch':
                    ...
                    this.outletDevices[deviceId][k] = v === 'false' ? false : !!v
                    break
                  default:
                    logRemove(key + '.' + k)
                    break
              ...
              for (const [k, v] of Object.entries(x)) {
                switch (k) {
                  ...
                  case 'scaleBattery':
                    ...
                    this.sensorDevices[deviceId][k] = v === 'false' ? false : !!v
                    break
                  ...
                  case 'sensorTimeDifference': {
                    ...
                    if (isNaN(intVal)) {
                      logDefault(key + '.' + k, this.consts.defaultValues[k])
                      this.sensorDevices[deviceId][k] = this.consts.defaultValues[k]
                    } else if (intVal < this.consts.minValues[k]) {
                      logIncrease(key + '.' + k, this.consts.minValues[k])
                      this.sensorDevices[deviceId][k] = this.consts.minValues[k]
                    } else {
                      this.sensorDevices[deviceId][k] = intVal
                    }
                    break
                  }
                  default:
              ...
              for (const [k, v] of Object.entries(x)) {
                switch (k) {
                  ...
                  case 'showAsOutlet':
                    ...
                    this.singleDevices[deviceId][k] = v === 'false' ? false : !!v
                    break
                  default:
                    logRemove(key + '.' + k)
                    break
              ...
              for (const [k, v] of Object.entries(x)) {
                switch (k) {
                  ...
                  case 'hideSwitch':
                    ...
                    this.thDevices[deviceId][k] = v === 'false' ? false : !!v
                    break
                  case 'offset': {
                    if (typeof v === 'string') {
                      logQuotes(key + '.' + k)
                    ...
                    if (isNaN(numVal)) {
                      ...
                      this.thDevices[deviceId][k] = numVal
                    }
                    break
                  }
                  default:
    ...
    try {
      ...
      if (this.consts.devices.curtain.includes(device.extra.uiid)) {
        ...
        rfMap.forEach(subDevice => {
          ...
          if (['1', '2', '3', '4'].includes(subDevice.type)) {
            ...
            subType = this.rfSensors[fullDeviceId]
              ...
              : 'motion'
            const sensorTimeLength = this.rfSensors[fullDeviceId] &&
              this.rfSensors[fullDeviceId].sensorTimeLength
              ? this.rfSensors[fullDeviceId].sensorTimeLength
              : this.consts.defaultValues.sensorTimeLength
            const sensorTimeDiff = this.rfSensors[fullDeviceId] &&
              this.rfSensors[fullDeviceId].sensorTimeDifference
              ? this.rfSensors[fullDeviceId].sensorTimeDifference
        ...
        ******************/
        ...
        UNKNOWN DEVICES
        **************/
        this.log.warn('[%s] %s.', device.name, this.messages.devNotSupYet)
        return
        /*************/
      }
      ...
      if (
        !this.hideMasters.includes(device.deviceid) &&
