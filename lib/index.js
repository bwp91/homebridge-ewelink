...
class eWeLinkPlatform {
  ...
  initialiseDevice (device) {
    try {
      ...
      if (this.consts.devices.curtain.includes(uiid)) {
        ...
        ***********************/
        for (let i = 0; i <= this.consts.supportedDevices[uiid]; i++) {
          ...
          subAccessory.context.reachableLAN = lanClient && lanDevices.has(device.deviceid) &&
        ...
        ******************/
        accessory = devicesInHB.get(uuid) || this.addAccessory(device, device.deviceid + 'SWX')
        accessory.control = new (require('./device/zigbee/sensor-contact'))(this, accessory, devicesInHB)
        /*****************/
      ...
      accessory.context.reachableLAN = lanClient && lanDevices.has(device.deviceid) &&
        lanDevices.get(device.deviceid).ip
  ...
  configureAccessory (accessory) {
    ...
    try {
      ...
      if (this.api && accessory.context.firmware) {
        accessory.getService(this.api.hap.Service.AccessoryInformation).updateCharacteristic(
          this.api.hap.Characteristic.FirmwareRevision,
