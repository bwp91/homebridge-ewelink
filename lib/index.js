    ...
    try {
      ...
      if (Object.keys(this.config.bridgeSensors || []).length > 0) {
        this.config.bridgeSensors
          .filter(s => this.helpers.hasProperty(s, 'fullDeviceId') && this.devicesInHB.has(s.fullDeviceId))
        ...
        ***********************/
        for (let i = 0; i <= this.helpers.chansFromUiid[device.extra.uiid]; i++) {
          if ((this.config.hideChanFromHB || '').split(',').includes(device.deviceid + 'SW' + i)) {
            ...
            oAccessory.getService(this.Service.AccessoryInformation)
              .setCharacteristic(this.Characteristic.FirmwareRevision, device.params.fwVersion || PLUGIN.version)
          }
