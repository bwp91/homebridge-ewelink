...
class eWeLinkPlatform {
    ...
    try {
      ...
      try {
        ...
        if (this.config.debug) {
          ...
          this.log.warn('Could not setup storage client as %s.', eText)
        }
      }
      ...
      if (this.config.username && this.config.password) {
        ...
        try {
          ...
          // Set up the WS client, get the user WS host, login, and setup the listener
        ...
        if (storageClient) {
          try {
            ...
            if (this.config.debug) {
              const eText = this.funcs.parseError(err)
              this.log.warn('Could not clear storage folder as %s.', eText)
            }
          }
        }
      } else {
        ...
        if (this.config.mode === 'lan' && storageClient) {
          try {
            ...
            if (this.config.debug) {
              const eText = this.funcs.parseError(err)
              this.log.warn('Could not read persist folder as %s.', eText)
            }
          }
        }
      }

      // Set up the LAN client, scan for device, start monitoring and setup the listener
  ...
  async initialiseDevice (device) {
    try {
      ...
      if (storageClient && this.consts.devices.lanOut.includes(uiid)) {
        try {
