...
class eWeLinkPlatform {
  ...
  initialiseDevice (device) {
    ...
    try {
      // Remove old sub accessories for Accessory Simulations or DUALR3 in motor mode
      if (
        this.simulations[device.deviceid] ||
        (device.extra.uiid === 126 && device.params.workMode === 2)
      ) {
        for (let i = 0; i <= 4; i++) {
      ...
      if (this.consts.devices.curtain.includes(device.extra.uiid)) {
        ...
        ***********************/
        ...
        /**********************/
      } else if (device.extra.uiid === 126 && device.params.workMode === 2) {
        /*************************
        ...
        ***********************/
        ...
        /**********************/
      } else if (this.consts.devices.sensorTempHumi.includes(device.extra.uiid)) {
        /*************************
        ...
        *************************************/
        ...
        /************************************/
      } else if (this.consts.devices.lightRGB.includes(device.extra.uiid)) {
        /***********
        LIGHTS [RGB]
        ***********/
        accessory = this.devicesInHB.has(uuid)
        ...
        /**********/
      } else if (this.consts.devices.lightCCT.includes(device.extra.uiid)) {
        /***********
        LIGHTS [CCT]
        ***********/
        accessory = this.devicesInHB.has(uuid)
        ...
        /**********/
      } else if (this.consts.devices.lightRGBCCT.includes(device.extra.uiid)) {
        /*****************
        LIGHTS [RGB & CCT]
        *****************/
        accessory = this.devicesInHB.has(uuid)
        ...
        /****************/
      } else if (this.consts.devices.lightDimmer.includes(device.extra.uiid)) {
        /**************
        LIGHTS [DIMMER]
        **************/
        accessory = this.devicesInHB.has(uuid)
        ...
        ***********************/
        for (let i = 0; i <= this.consts.supportedDevices[device.extra.uiid]; i++) {
          ...
          if (this.hideChannels.includes(device.deviceid + 'SW' + i)) {
            ...
            oAccessory = this.devicesInHB.has(uuidsub)
          ...
          oAccessory.context.ip = this.lanClient && oAccessory.context.reachableLAN
            ? this.lanDevices.get(device.deviceid).ip
            : false
          oAccessory.control = oAccessory.context.eweUIID === 126
            ? new (require('./device/switch-double'))(this, oAccessory)
            : new (require('./device/switch-multi'))(this, oAccessory)
          ...
          this.devicesInHB.set(oAccessory.UUID, oAccessory)
          if (i === 0) {
        ...
        rfMap.forEach(subDevice => {
          ...
          switch (subDevice.type) {
            ...
            default: {
              ...
              this.log.warn('[%s-%s] %s', device.extra.uiid, subDevice.type || '?', data)
              return
            }
          }
          ...
          if ((oAccessory = this.devicesInHB.get(uuidsub))) {
        ...
        *****************/
        ...
        /****************/
      } else if (this.consts.devices.zbLightCCT.includes(device.extra.uiid)) {
        /**************
        ZB LIGHTS [CCT]
        **************/
        accessory = this.devicesInHB.has(uuid)
        ...
        *************/
        ...
        return
        /************/
      } else if (this.consts.devices.cannotSupport.includes(device.extra.uiid)) {
        /*******************************
        ...
        **********************/
        this.log.warn('[%s] %s:', device.name, this.messages.devNotSupYet)
        this.log.warn('[%s] %s', device.extra.uiid || '?', JSON.stringify(device.params))
        return
        /*********************/
      }
      ...
      const str = accessory.context.reachableLAN
        ...
        : this.consts.devices.lanIn.includes(device.extra.uiid)
      ...
      if (
        ...
        !this.consts.devices.lanOut.includes(device.extra.uiid)
      ) {
        this.log.warn('[%s] %s.', device.name, this.messages.devNoControl)
      }
    } catch (err) {
      // Catch any errors during initialisation
      ...
      if (res !== 'ok') {
        ...
        if (this.wsClient) {
          ...
          if (accessory.context.reachableWAN) {
            ...
            throw new Error(this.messages.unreachable)
          }
        } else {
          // Device isn't online via WS so report the error back
          const eText = [this.messages.lanDisabled, this.messages.lanNotSup].includes(res)
            ? res
            : this.messages.unreachable + ' [' + res + ']'
      ...
      if (reachableChange && accessory.context.hbDeviceId.substr(-1) !== 'X') {
        ...
        for (let i = 1; i <= accessory.context.channelCount; i++) {
          ...
          if (this.devicesInHB.has(uuid)) {
            ...
            const oAccessory = this.devicesInHB.get(uuid)
