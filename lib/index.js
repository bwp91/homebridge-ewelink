...
class eWeLinkPlatform {
                ...
                switch (k) {
                  ...
                  case 'type': {
                    ...
                    if (typeof v !== 'string' || !inSet) {
                      ...
                      this.rfDevices[id][k] = inSet ? v : this.consts.defaultValues[k]
                    }
                    break
                  }
  ...
  async initialiseDevice (device) {
    try {
        ...
        *********************/
        ...
        rfMap.forEach(subDevice => {
          ...
          if ((subAccessory = devicesInHB.get(uuidsub))) {
            // Check for any changes given RF structure (ie added/deleted RF subdevices)
            if (subAccessory.context.swNumber !== swNumber) {
      ...
      const str =
        accessory.context.reachableLAN && this.consts.devices.lan.includes(uiid)
          ? this.ipOverride[device.deviceid]
            ? this.lang.foundWithManIP + ' [' + this.ipOverride[device.deviceid] + ']'
