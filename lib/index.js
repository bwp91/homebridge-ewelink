...
class eWeLinkPlatform {
  ...
  initialiseDevice (device) {
    ...
    try {
        ...
        ***********************/
        for (let i = 0; i <= this.consts.chansFromUiid[device.extra.uiid]; i++) {
          ...
          if (this.config.hideChanFromHB.includes(device.deviceid + 'sw' + i)) {
            ...
            // Set the firmware version for the sub accessory
      ...
      // Update the current firmware version if the accessory isn't hidden
      if (
  ...
  async sendDeviceUpdate (accessory, params) {
    // Add a random delay of [1, 250]ms to avoid multiple http/ws requests at once
    ...
    if (res !== 'ok') {
      ...
      if (this.wsClient && accessory.context.reachableWAN) {
        ...
        // Device is online via WS so sent the update
  ...
  async receiveDeviceUpdate (device) {
    ...
    if ((
      ...
      try {
        ...
        if (this.config.debug) {
          const str = JSON.stringify(device.params)
          this.log('[%s] %s %s.', device.name, this.messages.recNew, str)
        }
