...
class eWeLinkPlatform {
    ...
    try {
      ...
      if (this.helpers.devicesCurtain.includes(device.extra.uiid)) {
        ...
        *********************/
        ...
        rfMap.forEach(subDevice => {
          ...
          if (['1', '2', '3', '4'].includes(subDevice.type)) {
            ...
            this.log(' → [%s] has an unsupported device type [%s].', device.name, subDevice.type || '?')
            return
          }
          subExtraContext = {
        ...
        ******/
        ...
        CAMERAS
        ******/
        this.log(' → [%s] plugin does not support the eWeLink camera.', device.name)
        return
        /*****/
      } else {
        /**********
  ...
  async sendDeviceUpdate (accessory, params) {
    ...
    if (this.debugReqRes) {
      this.log.warn(
        '[%s] sending update - this text is yellow for clarity.\n%s',
        accessory.displayName,
    ...
    if ((accessory = this.devicesInHB.get(deviceId + 'SWX') || this.devicesInHB.get(deviceId + 'SW0'))) {
      if (this.debugReqRes) {
        this.log(
          '[%s] update received.\n%s',
          accessory.displayName,
          JSON.stringify(device.params, null, 2)
        )
      } else if (this.debug) {
      ...
      try {
        ...
        if (this.debugReqRes) {
          this.log(
            '[%s] update received for new device.\n%s',
            device.name,
            JSON.stringify(device.params, null, 2)
          )
        } else if (this.debug) {
