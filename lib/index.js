...
class eWeLinkPlatform {
  ...
  async sendDeviceUpdate (accessory, params) {
    ...
    if (this.debugReqRes || this.debug) {
  ...
  async deviceUpdateError (accessory, err, requestRefresh) {
    this.log.warn('[%s] sending update failed as %s.', accessory.displayName, this.debug ? err : err.message)
    ...
    if (accessory.context.reachableWAN) {
      try {
        ...
        this.log.warn('[%s] reverting to previous state.', accessory.displayName)
      } catch (err) {}
    }
  }
}
