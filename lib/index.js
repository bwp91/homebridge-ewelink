...
class eWeLinkPlatform {
      ...
      switch (key) {
        case 'bridgeSensors':
          if (Array.isArray(val) && val.length > 0) {
            val.forEach(x => {
              ...
              const fullDeviceId = this.funcs.parseDeviceId(x.fullDeviceId)
              this.rfSensors[fullDeviceId] = {}
                ...
                switch (k) {
                  ...
                  case 'sensorTimeLength': {
                    ...
                    if (isNaN(intVal)) {
                      ...
                      this.rfSensors[fullDeviceId][k] = this.consts.defaultValues[k]
                    } else if (intVal < this.consts.minValues[k]) {
                      logIncrease(key + '.' + k, this.consts.minValues[k])
                      this.rfSensors[fullDeviceId][k] = this.consts.minValues[k]
                    } else {
                      this.rfSensors[fullDeviceId][k] = intVal
                    }
                    break
                  }
                  case 'type':
                    if (typeof v !== 'string' || v === '') {
                      logIgnore(key + '.' + k)
                    } else if (!this.consts.allowed.sensors.includes(x.type)) {
                      logIgnore(key + '.' + k)
                    } else {
                      this.rfSensors[fullDeviceId][k] = v
                    }
                    break
                  default:
                    logRemove(key + '.' + k)
        ...
        case 'countryCode': {
          ...
          if (isNaN(intVal)) {
            ...
            this.config.countryCode = '+' + val.toString().replace(/[\s+]+/g, '')
          }
          break
        }
        case 'debug':
                ...
                switch (k) {
                  ...
                  case 'obstructId':
                    if (typeof v !== 'string' || v === '') {
                      ...
                      this.simulations[deviceId].obstructId = parsed
                      this.obstructSwitches[parsed] = deviceId
                    }
                    break
                  case 'operationTime': {
                    if (typeof v === 'string') {
                    ...
                    if (isNaN(intVal)) {
                      logDefault(key + '.' + k, this.consts.defaultValues[k])
                      this.simulations[deviceId][k] = this.consts.defaultValues[k]
                    } else if (intVal < this.consts.minValues[k]) {
                      logIncrease(key + '.' + k, this.consts.minValues[k])
                      this.simulations[deviceId][k] = this.consts.minValues[k]
                    } else {
                      this.simulations[deviceId][k] = intVal
                    }
                    break
                  }
                  case 'sensorId':
                    if (typeof v !== 'string' || v === '') {
                      ...
                      const parsed = this.funcs.parseDeviceId(v)
                      this.simulations[deviceId].sensorId = parsed
                    }
                    break
                  case 'setup':
                    if (typeof v !== 'string' || v === '') {
                      ...
                      this.simulations[deviceId][k] = v
                    }
                    break
                  case 'type':
                    if (typeof v !== 'string' || v === '') {
                      logIgnore(key + '.' + k)
                    } else if (!this.consts.allowed.groups.includes(x.type)) {
                      logIgnore(key + '.' + k)
                    } else {
                      this.simulations[deviceId][k] = v
                    }
        ...
        case 'switchAsOutlet':
        case 'outletAsSwitch':
        case 'hideLightFromFan':
        case 'hideSwitchFromTH':
        case 'resetRFBridge':
        case 'bulbB02BA60':
        case 'bulbB02FST64':
          this.config[key] = []
          if (typeof val !== 'string' || val === '') {
            ...
            items.forEach(item => {
              this.config[key].push(this.funcs.parseDeviceId(val))
            })
          }
          break
        ...
        case 'ipOverride':
          ...
          break
        case 'mode': {
          const inSet = ['auto', 'wan', 'lan'].includes(val)
                ...
                switch (k) {
                  ...
                  case 'offset': {
                    ...
                    if (isNaN(intVal)) {
                      ...
                      this.thTempOffset[deviceId] = intVal
                    }
                    break
                  }
                  default:
  ...
  initialiseDevice (device) {
    ...
    try {
      ...
      if (this.consts.devices.curtain.includes(device.extra.uiid)) {
        ...
        ***********************/
        const cache = this.consts.defaultCurtainCache
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX', false, cache)
        ...
        ****************************/
        const cache = this.consts.defaultBlindCache
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX', false, cache)
        ...
        ****************************************/
        const cache = this.consts.defaultGarageCache
        ...
        ****************************************/
        const cache = this.consts.defaultGarageTwoCache
        ...
        *****************************************/
        const cache = this.consts.defaultGarageFourCache
        ...
        ***********************************/
        ...
        accessory.control = new (require(instance))(this, accessory)
        /**********************************/
      } else if (this.consts.devices.sensor.includes(device.extra.uiid)) {
        /*******************
        SENSORS [SONOFF DW2]
        *******************/
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
        ...
        ********/
        ...
        this.consts.devices.thermostat.includes(device.extra.uiid)
      ) {
        /*********************************
        ...
        ***********************/
        const cache = { sensorType: device.params.sensorType }
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX', false, cache)
        ...
        (this.consts.devices.singleSwitch.includes(device.extra.uiid) && this.consts.devices.singleSwitchOutlet.includes(device.productModel))
      ) {
        /******
        OUTLETS
        ******/
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX')
        accessory.control = this.config.outletAsSwitch.includes(device.deviceid)
          ? new (require('./device/switch-single'))(this, accessory)
          : new (require('./device/outlet'))(this, accessory)
        /*****/
      } else if (this.consts.devices.usb.includes(device.extra.uiid)) {
        /**********
        USB OUTLETS
        **********/
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
        ...
        /*********/
      } else if (this.consts.devices.scm.includes(device.extra.uiid)) {
        /*********************************
        ...
        ************************/
        accessory = this.devicesInHB.has(device.deviceid + 'SWX')
          ? this.devicesInHB.get(device.deviceid + 'SWX')
          : this.addAccessory(device, device.deviceid + 'SWX')
        accessory.control = this.config.switchAsOutlet.includes(device.deviceid)
          ? new (require('./device/outlet'))(this, accessory)
          : new (require('./device/switch-single'))(this, accessory)
        /***********************/
        ...
        ***********************/
        for (let i = 0; i <= this.consts.chansFromUiid[device.extra.uiid]; i++) {
          if (this.config.hideChanFromHB.includes(device.deviceid + 'sw' + i)) {
        ...
        *********************/
        ...
        rfMap.forEach(subDevice => {
          ...
          if (['1', '2', '3', '4'].includes(subDevice.type)) {
            ...
            subType = this.rfSensors[fullDeviceId]
              ? this.rfSensors[fullDeviceId].type
              : 'motion'
            if (this.rfSensors[fullDeviceId]) {
              sensorTimeLength = this.rfSensors[fullDeviceId].sensorTimeLength
            }
          } else {
        ...
        ******************/
        ...
        SONOFF CAMERAS
        *************/
        this.log('[%s] %s.', this.messages.sonoffCamera, device.name)
        return
        /************/
        ...
        EWELINK APP CAMERAS
        ******************/
        this.log('[%s] %s.', this.messages.devNotSup, device.name)
        return
        /*****************/
      } else {
        /**************
        UNKNOWN DEVICES
        **************/
        this.log.warn('[%s] %s.', this.messages.devNotSupYet, device.name)
        return
        /*************/
      }
      ...
      if (
        ...
        !this.obstructSwitches[device.deviceid]
      ) {
  ...
  addAccessory (device, hbDeviceId, hidden = false, extraContext = {}, type = '') {
    ...
    try {
      ...
      const channelCount = type === 'rf_pri'
        ...
        : this.consts.chansFromUiid[device.extra.uiid]
  ...
  async sendDeviceUpdate (accessory, params) {
    ...
    await this.funcs.sleep(Math.floor(Math.random() * 250 + 1))
    ...
    const errorOne = 'LAN mode is disabled'
    const errorTwo = 'LAN mode is not supported for this device'
    ...
    if (res !== 'ok') {
      ...
      if (this.wsClient && accessory.context.reachableWAN) {
        ...
        throw new Error(
          [errorOne, errorTwo].includes(res)
            ? 'it is unreachable'
            : 'it is unreachable [' + res + ']'
        )
      }
    }
  }
  ...
  async receiveDeviceUpdate (device) {
    ...
    if ((accessory = this.devicesInHB.get(
      deviceId + 'SWX') || this.devicesInHB.get(deviceId + 'SW0')
    )) {
      if (this.config.debug) {
        this.log(
          '[%s] receiving update %s.',
      ...
      if (device.params.updateSource === 'WS') {
        ...
        if (device.params.online !== accessory.context.reachableWAN) {
          ...
          this.log(
            '[%s] reported [%sline] via WS.',
            accessory.displayName,
            accessory.context.reachableWAN ? 'on' : 'off'
          )
      ...
      try {
        ...
        const errToShow = this.config.debug
          ? ':\n' + err
      ...
      try {
        ...
        if (this.config.debug) {
          this.log(
            '[%s] receiving update for new device %s.',
            device.name,
            JSON.stringify(device.params)
          )
        }
        ...
        const errToShow = this.config.debug
          ...
          : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
        this.log.warn(
          '[%s] restart Homebridge to add new device, failed to add automatically as%s',
          deviceId,
          errToShow
        )
      }
