        ...
        case 'groups':
          if (Array.isArray(val) && val.length > 0) {
            val.forEach(x => {
              ...
              this.log.warn(
                '[%s - %s] Please recreate this simulation in the Single/Multi Channel/TH Devices section of the configuration and remove this entry from the Accessory Simulations section.',
                id,
                x.type
              )
              for (const [k, v] of entries) {
                switch (k) {
                  ...
                  case 'obstructId':
                    if (typeof v !== 'string' || v === '') {
                      ...
                      if (!this.funcs.hasProperty(this.obstructSwitches, parsed)) {
                        this.obstructSwitches[parsed] = id
                      }
                    }
                    break
                  ...
                  case 'type': {
                    ...
                    const inSet = this.consts.allowed[k1].includes(v)
                    if (typeof v !== 'string' || !inSet) {
                      logIgnore(key + '.' + id + '.' + k1)
                    } else {
  ...
  async initialiseDevice (device) {
    try {
      ...
      if (uiid === 126 && device.params.workMode === 2) {
        ...
        ************************************/
        const sim = Object.values(this.deviceConf).find(el => el.sensorId === device.deviceid)
        let instance
        if (sim.hideSensor) {
