...
class eWeLinkPlatform {
    ...
    try {
      ...
      devicesInHB.forEach(accessory => {
        if (!deviceList.some(el => el.deviceid === accessory.context.eweDeviceId)) {
          // Keep compatible LAN devices that aren't ignored (for LAN-only mode)
          if (
            this.config.mode === 'lan' &&
            this.consts.devices.lanOut.includes(accessory.context.eweUIID) &&
            !this.config.ignoredDevices.includes(accessory.context.eweDeviceId)
          ) {
            // We only want to initialise the primary accessory for each device
            if (['X', '0'].includes(accessory.context.switchNumber)) {
              this.initialiseDevice(accessory.context.fullInfo)
            }
          } else {
  ...
  initialiseDevice (device) {
    try {
      ...
      if (this.consts.devices.curtain.includes(uiid)) {
        ...
        ***********************/
        ...
        for (let i = 0; i <= this.consts.supportedDevices[uiid]; i++) {
          ...
          subAccessory.context.macAddress =
            device.extra && device.extra.staMac ? device.extra.staMac : false
        ...
        *********************/
        ...
        rfMap.forEach(subDevice => {
          ...
          const disableDeviceLogging =
            this.rfDevices[fullDeviceId] && this.rfDevices[fullDeviceId].overrideDisabledLogging
              ? false
          ...
          switch (subDevice.type) {
            ...
            case '6':
            case '7':
              subType = this.rfDevices[fullDeviceId] ? this.rfDevices[fullDeviceId].type : 'motion'
              sensorTimeLength =
                this.rfDevices[fullDeviceId] && this.rfDevices[fullDeviceId].sensorTimeLength
                  ? this.rfDevices[fullDeviceId].sensorTimeLength
                  : this.consts.defaultValues.sensorTimeLength
              sensorTimeDiff =
                this.rfDevices[fullDeviceId] && this.rfDevices[fullDeviceId].sensorTimeDifference
      ...
      accessory.context.fullInfo = device
      ...
      if (this.config.mode === 'lan' && !this.consts.devices.lanOut.includes(uiid)) {
        this.log.warn('[%s] %s.', accessory.displayName, this.lang.devNoControl)
      }
    } catch (err) {
      // Catch any errors during initialisation
      const eText = this.funcs.parseError(err)
  ...
  async sendDeviceUpdate (accessory, params, log = true) {
    ...
    return await queue.add(async () => {
      ...
      const res = !lanClient
        ...
        : !this.consts.devices.lanOut.includes(accessory.context.eweUIID)
        ? this.lang.lanNotSup
        : await lanClient.sendUpdate(payload, accessory.context)
    ...
    if ((accessory = devicesInHB.get(uuid1) || devicesInHB.get(uuid2))) {
      ...
      if (device.params.updateSource === 'WS') {
        ...
        if (device.params.online !== accessory.context.reachableWAN) {
          ...
          const status = accessory.context.reachableWAN ? this.lang.repOnline : this.lang.repOffline
      ...
      if (device.params.updateSource === 'LAN') {
        ...
        if (
          accessory.context.reachableLAN &&
          device.params.ip &&
          device.params.ip !== accessory.context.ip
        ) {
