    ...
    try {
      ...
      if (this.wsClient) {
        this.wsRefresh = setInterval(async () => {
          try {
            if (this.debug) {
              this.log('Refreshing WS connection...')
            }
            await this.wsClient.getHost()
    ...
    try {
      ...
      if (this.helpers.devicesCurtain.includes(device.extra.uiid)) {
        ...
        ********/
        ...
        this.cusG.has(device.deviceid) &&
        this.cusG.get(device.deviceid).type === 'thermostat' &&
        this.helpers.devicesThermostat.includes(device.extra.uiid)
      ) {
        /*********************************
