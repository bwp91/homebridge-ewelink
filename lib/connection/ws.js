...
module.exports = class connectionWS {
      ...
      this.wsClient.onUnpackedMessage.addListener(msg => {
        ...
        if (msg.action) {
          ...
          switch (msg.action) {
            case 'update':
            case 'sysmsg': {
              if (msg.action === 'sysmsg' && this.funcs.hasProperty(msg.params, 'online')) {
        ...
        } else if (this.funcs.hasProperty(msg, 'error') && [0, 406].includes(msg.error)) {
  ...
  async requestUpdate (accessory) {
    ...
    if (this.consts.devices.skipUpdateRequest.includes(accessory.context.eweUIID)) {
      return
    }
