...
module.exports = class connectionWS {
  ...
  async getWSHost () {
    ...
    try {
      ...
      if (err.code && this.consts.httpRetryCodes.includes(err.code)) {
        ...
        return await this.getWSHost()
      } else {
        // It's not a eWeLink problem so report the error back
        this.log.warn('An error occurred during getWSHost().')
        throw err
      }
    }
  }
  ...
  async login () {
    ...
    try {
      ...
      const wsHost = await this.getWSHost()
      ...
      this.wsClient.onClose.addListener(async e => {
        ...
        if (this.debug) {
          this.log.warn('%s [%s].', this.messages.wsReconnectError, e)
        }
      ...
      this.wsClient.onError.addListener(async e => {
        // Log that the web socket has closed if appropriate
        if (this.debug) {
          this.log.warn('%s [%s].', this.messages.wsReconnectClose, e)
        }
      ...
      if (this.debug) {
        this.log('%s: %s.', this.messages.wsLoginErrRecon, errToShow)
      }
      ...
      try {
        ...
        switch (res.error) {
          case 0:
            ...
            return
          case 504:
            throw new Error(this.messages.wsReqTimeout + ' [504]')
          default:
            // An error has occurred
            throw new Error(this.messages.wsUnkRes + ' [' + res.error + ']')
        }
      } catch (err) {
