...
module.exports = class connectionWS {
  ...
  login () {
    this.wsp = new (require('websocket-as-promised'))(
    ...
    this.wsp.onOpen.addListener(async () => {
      ...
      try {
        ...
        this.log.warn('WS login failed as %s.', this.debug ? err : err.message)
      }
    })
  ...
  async sendUpdate (json) {
    ...
    if (this.wsp && this.wsIsOpen) {
      try {
        ...
        return new Error('device update failed as ' + this.debug ? err : err.message)
      }
    } else {
