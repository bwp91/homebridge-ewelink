  ...
  async sendUpdate (json, context) {
    try {
      ...
      if (json.params.brightness) {
        ...
        // iFan02 product model = iFan02
        // iFan03 product model = iFan or iFan03
        // both have uiid 34
        // iFan02 uses 'switches'
        // iFan03 uses fan, light, speed
        // model obtained here with context.eweModel
        if (context.eweModel === 'iFan03') {
          params.light = json.params.switches[0].switch
          params.fan = json.params.switches[1].switch
          switch (json.params.switches[2].switch + json.params.switches[3].switch) {
            case 'offoff':
              params.speed = 1
              break
            case 'onoff':
        ...
        suffix = 'switch'
      } else if (json.params.uiActive && context.uiid !== 15) {
        // Not supported at least for TH10/16
