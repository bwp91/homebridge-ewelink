...
module.exports = class connectionLAN {
      ...
      if (packet.answers) {
        packet.answers
          ...
          .forEach(value => {
            ...
            if (!this.helpers.devicesLAN.includes(deviceInfo.uiid)) return
            ...
            try {
              ...
              this.log.warn('[%s] LAN message received could not be read [%s]:\n%s', rdata.id, e.pText)
              return
            }
  ...
  addDeviceToMap (device) {
    this.deviceMap.set(device.deviceid, {
      apiKey: device.devicekey,
      online: this.helpers.devicesLAN.includes(device.extra.uiid) && this.helpers.hasProperty(this.ipOverride, device.deviceid),
      ip: this.helpers.devicesLAN.includes(device.extra.uiid) && this.helpers.hasProperty(this.ipOverride, device.deviceid)
  ...
  async closeConnection () {
    ...
    if (this.debug || this.debugReqRes) this.log('LAN monitoring gracefully stopped.')
  }
}
