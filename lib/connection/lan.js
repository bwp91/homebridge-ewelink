...
module.exports = class connectionLAN {
  constructor (config, log, helpers, devices) {
    this.log = log
    this.config = config
    ...
    devices.forEach(device => {
      this.deviceMap.set(device.deviceid, {
        ...
        online: this.helpers.hasProperty(this.config.ipOverride, device.deviceid),
        ip: this.helpers.hasProperty(this.config.ipOverride, device.deviceid) ? this.config.ipOverride[device.deviceid] : null,
        lastIV: null
      })
    })
  ...
  async getHosts () {
    ...
    res.forEach(device => {
      ...
      if ((d = this.deviceMap.get(deviceId))) {
        if (!this.helpers.hasProperty(this.config.ipOverride, deviceId)) {
      ...
      if (packet.answers) {
        packet.answers
          ...
          .forEach(value => {
            ...
            let params
            if (packet.address !== deviceInfo.ip && !this.helpers.hasProperty(this.config.ipOverride, rdata.id)) {
