...
module.exports = class connectionLAN {
  ...
  async getHosts () {
    ...
    if (this.debug) {
      this.log('%s.', this.messages.lanStarting)
    }
    const res = await this.dns.discover({
      name: '_ewelink._tcp.local'
    })
    if (this.debug) {
      this.log('%s.', this.messages.lanStarted)
    }
  ...
  async startMonitor () {
    ...
    this.dns.ondata = packet => {
      try {
        ...
        this.log.warn('%s %s.', this.messages.cantParsePacket, eText)
      }
    }

    // Start the DNS packet monitoring
    await this.dns.startMonitoring()
    this.log('%s.', this.messages.lanMonitor)
  }
  ...
  async sendUpdate (json) {
    try {
      ...
      if (!this.deviceMap.get(json.deviceid).ip) {
        throw new Error(this.messages.devNotReachLAN)
      }
      let apiKey
      let suffix
      const params = {}

      // For now only /dimmable (for D1), /switch /switches are supported
      if (json.params.brightness) {
        ...
        throw new Error(this.messages.devNotConfLAN)
      }

      // Check we have the device API key
      if (!(apiKey = this.deviceMap.get(json.deviceid).apiKey)) {
        throw new Error(this.messages.devNoAPIKey)
      }
      ...
      const res = await this.axios({
        ...
        data,
        timeout: this.mode === 'lan' ? 10000 : 3000
      })
  ...
  async closeConnection () {
    ...
    if (this.debug) {
      this.log('%s.', this.messages.stoppedLAN)
    }
  }
}
