...
module.exports = class connectionHTTP {
  ...
  async getHost () {
    ...
    try {
      ...
      dataToSign = crypto.createHmac('sha256', this.helpers.appSecret).update(dataToSign).digest('base64')
  ...
  async login () {
    ...
    // *** Set up the request signature *** \\
    const dataToSign = crypto.createHmac('sha256', this.helpers.appSecret).update(JSON.stringify(data)).digest('base64')
