...
module.exports = class connectionHTTP {
  ...
  async getHost () {
    ...
    try {
      ...
      dataToSign = this.crypto.createHmac('sha256', this.consts.appSecret).update(dataToSign).digest('base64')
      ...
      if (this.funcs.hasProperty(err, 'code') && this.consts.httpRetryCodes.includes(err.code)) {
  ...
  async login () {
    ...
    // Set up the request signature
    const dataToSign = this.crypto.createHmac('sha256', this.consts.appSecret).update(JSON.stringify(data)).digest('base64')
      ...
      if (body.data.thingList && body.data.thingList.length > 0) {
        body.data.thingList.forEach(d => {
          ...
          if (d.itemData && d.itemData.extra && d.itemData.extra.uiid && !this.ignoredDevices.includes(d.itemData.deviceid)) {
            ...
            if (d.itemData.extra.uiid === 102 || this.obstructSwitches[d.itemData.deviceid]) {
