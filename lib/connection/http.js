...
module.exports = class connectionHTTP {
  ...
  async login () {
    try {
      ...
      const dataToSign = crypto
        .createHmac('sha256', this.consts.appSecret)
      ...
      } else if (body.error === 10001 && !this.triedBase64) {
  ...
  async getHomes () {
    ...
    try {
      ...
      body.data.familyList.forEach(home => this.homeList.push(home.id))
    } catch (err) {
      // Check to see if it's a eWeLink server problem and we can retry
      if (err.code && this.consts.httpRetryCodes.includes(err.code)) {
        // Retry if another attempt could be successful
    ...
    try {
      ...
      fullDeviceList.forEach(d => {
        ...
        if (
          ...
          if (this.mode === 'lan' && !this.consts.devices.lan.includes(d.itemData.extra.uiid)) {
            return
          }
