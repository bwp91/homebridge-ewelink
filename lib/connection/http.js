...
module.exports = class connectionHTTP {
  constructor (platform) {
    ...
    this.username = platform.config.username.toString()
    this.password = platform.config.password.toString()
    this.hideDevFromHB = (platform.config.hideDevFromHB || '').split(',')
    this.cCode = '+' + platform.config.countryCode.toString().replace('+', '').replace(' ', '')
  ...
  async getHost () {
    ...
    try {
      ...
      if (this.debug || this.debugReqRes) this.log('Sending HTTP getHost() request.')
      ...
      if (this.debug || this.debugReqRes) this.log('HTTP API host received [%s].', this.httpHost)
    } catch (err) {
  ...
  async login () {
    ...
    // *** Log the data depending on the debug setting *** \\
    if (this.debug || this.debugReqRes) this.log('Sending HTTP login() request.')
    ...
    if (
      ...
      if (this.debug || this.debugReqRes) this.log('New HTTP API host received [%s].', this.httpHost)
