...
module.exports = class connectionHTTP {
  ...
  async login () {
    try {
      ...
      if (err.code && this.consts.httpRetryCodes.includes(err.code)) {
        // Retry if another attempt could be successful
        this.log.warn('%s [login() - %s].', this.messages.httpRetry, err.code)
        await this.funcs.sleep(30000)
        return await this.login()
      } else {
        // It's not a eWeLink problem so report the error back
        this.log.warn('%s.', this.messages.errLogin)
        throw err
      }
    }
  }
  ...
  async getDevices () {
    ...
    try {
      ...
      if (err.code && this.consts.httpRetryCodes.includes(err.code)) {
        // Retry if another attempt could be successful
        this.log.warn('%s [getDevices() - %s].', this.messages.httpRetry, err.code)
        await this.funcs.sleep(30000)
        return await this.getDevices()
      } else {
        // It's not a eWeLink problem so report the error back
        this.log.warn('%s.', this.messages.errGetDevices)
        throw err
      }
    }
  }
  ...
  async getDevice (deviceId) {
    ...
    try {
      ...
      if (err.code && this.consts.httpRetryCodes.includes(err.code)) {
        // Retry if another attempt could be successful
        this.log.warn('%s [getDevice() - %s].', this.messages.httpRetry, err.code)
        await this.funcs.sleep(30000)
        return await this.getDevice(deviceId)
      } else {
        // It's not a eWeLink problem so report the error back
        this.log.warn('%s.', this.messages.errGetDevice)
        throw err
      }
    }
  }
