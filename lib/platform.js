/* jshint -W014, -W033, esthis.version: 9 */
...
module.exports = class eWeLinkPlatform {
  ...
  initialiseDevice (device) {
    ...
    try {
        ...
        for (let i = 0; i <= this.helpers.chansFromUiid[device.extra.uiid]; i++) {
          if ((this.config.hideChanFromHB || '').split(',').includes(device.deviceid + 'SW' + i)) {
            ...
            oAccessory
              ...
              .setCharacteristic(this.Characteristic.FirmwareRevision, device.params.fwthis.version)
          }
      ...
      // *** Update the current firmware this.version *** \\
    ...
    try {
      ...
      if (!hidden) {
        accessory
          ...
          .setCharacteristic(this.Characteristic.FirmwareRevision, device.params.fwthis.version || this.version)
